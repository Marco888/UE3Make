...
void UFactory::StaticConstructor()
{
	...
	new(GetClass(),TEXT("InContextCommand"),RF_Public)UStrProperty(CPP_PROPERTY(InContextCommand),TEXT(""),0);
	new(GetClass(),TEXT("OutOfContextCommand"),RF_Public)UStrProperty(CPP_PROPERTY(OutOfContextCommand),TEXT(""),0);
...
)
{
	...
	for( INT i=0; i<Factories.Num(); i++ )
	{
		...
		if( Factory->bCreateNew )
		{
			...
			{
				debugf( NAME_Log, TEXT("FactoryCreateNew: %ls with %ls (%i %i %ls)"), Class->GetName(), Factories(i)->GetClass()->GetName(), Factory->bCreateNew, Factory->bText, Filename );
		...
		else if( appStricmp(Filename,TEXT(""))!=0 )
		{
			if( Factory->bText )
			{
				debugf( NAME_Log, TEXT("FactoryCreateText: %ls with %ls (%i %i %ls)"), Class->GetName(), Factories(i)->GetClass()->GetName(), Factory->bCreateNew, Factory->bText, Filename );
				FString Data;
			...
			{
				debugf( NAME_Log, TEXT("FactoryCreateBinary: %ls with %ls (%i %i %ls)"), Class->GetName(), Factories(i)->GetClass()->GetName(), Factory->bCreateNew, Factory->bText, Filename );
				TArray<BYTE> Data;
	...
	if( !InFactory )
		for( INT i=0; i<Factories.Num(); i++ )
			delete Factories(i);
	Warn->Logf( TEXT("Missing or invalid file '%ls' for import"), Filename );
	return NULL;
	unguard;
}
