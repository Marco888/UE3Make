...
UExporter* UExporter::FindExporter( UObject* Object, const TCHAR* FileType )
{
	...
	// List all usable exporters.
	TMap<UClass*,UClass*> ExpMap;
			...
			if( Default->SupportedClass && Object->IsA(Default->SupportedClass) )
				for( INT i=0; i<Default->Formats.Num(); i++ )
					...
					||	appStricmp( *Default->Formats(i), TEXT("*") )==0 )
						ExpMap.Set(Default->SupportedClass,*It);
		}
	}

	// Look down the class tree for best exporter.
	for( UClass* C=Object->GetClass(); C; C=C->GetSuperClass() )
	{
		UClass* Res = ExpMap.FindRef(C);
		if( Res )
			return ConstructObject<UExporter>( Res );
	}
	return NULL;
	unguard;
}
...
void UExporter::ExportToArchive( UObject* Object, UExporter* InExporter, FArchive& Ar, const TCHAR* FileType )
{
	...
	if( !Exporter )
	{
		GWarn->Logf( TEXT("No %ls exporter found for %ls"), FileType, Object, Object->GetFullName() );
		return;
	}
...
void UExporter::ExportToOutputDevice( UObject* Object, UExporter* InExporter, FOutputDevice& Out, const TCHAR* FileType, INT Indent )
{
	...
	if( !Exporter )
	{
		GWarn->Logf( TEXT("No %ls exporter found for %ls"), FileType, Object->GetFullName() );
		return;
	}
	check(Object->IsA(Exporter->SupportedClass));
...
UBOOL UExporter::ExportToFile( UObject* Object, UExporter* InExporter, const TCHAR* Filename, UBOOL NoReplaceIdentical, UBOOL Prompt )
{
	...
	UBOOL Result = 0;
	debugf(TEXT("Exporting: %ls with extension %ls"),Filename,FileType);
	if( !Exporter )
	{
		Exporter = FindExporter( Object, FileType );
	}
	...
	if( Exporter->bText )
	{
		...
		if( NoReplaceIdentical )
		{
			...
			{
				debugf( TEXT("Not replacing %ls because identical"), Filename );
				Result = 1;
				goto Done;
			}
			if( Prompt )
			{
				if( !GWarn->YesNof( TEXT("The file '%ls' needs to be updated.  Do you want to overwrite the existing version?"), Filename ) )
				{
					Result = 1;
					goto Done;
				}
		...
		{
			GWarn->Logf( TEXT("Error exporting %ls: couldn't open file '%ls'"), Object->GetFullName(), Filename );
			goto Done;
		}
		Result = 1;
	}
