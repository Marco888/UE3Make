...
)
{
	...
	// Allocate memory.
	CacheMemory		 = Start ? (BYTE*)Start : NewTagged(TEXT("CacheMemory"))BYTE[BytesToAllocate];
	ItemMemory       = NewTagged(TEXT("CacheItems"))FCacheItem[MaxItems];
...
void FMemCache::Flush( QWORD Id, DWORD Mask, UBOOL IgnoreLocked )
{
	...
	else
	{
		...
		{
			// Make sure we flushed the entire cache and there is only one cache item remaining for each segment.
			check(CacheItems!=NULL);
			INT ExpectSegment=0;
	...
	if( ParseCommand(&Cmd,TEXT("DUMPCACHE")) )
	{
		for( FCacheItem* Item=CacheItems; Item!=LastItem; Item=Item->LinearNext )
		{
			...
			if( Item->Cost >= COST_INFINITE )
				...
				Descr=TEXT("Fresh");
			Ar.Logf( TEXT("%02X [%i]: %ls"), (BYTE)Item->Id, Item->LinearNext->Data - Item->Data, Descr );
		}
		return 1;
	}
	else return 0;
