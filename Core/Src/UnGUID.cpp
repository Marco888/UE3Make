...
{
    guard(get_ieee_node_identifier);
	unsigned32 ip32;
	unsigned16 random16;
...
get_system_time(unsigned64_t *guuid_time)
{
  guard(get_system_time);
  #if __PSX2_EE__
  #else
  struct timeval tp;
  ...
  ADD_64b_2_64b(&utc, &os_basetime_diff, guuid_time);
  #endif
  unguard;
}
