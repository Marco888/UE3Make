...
public:
	FArchive* CreateFileReader( const TCHAR* FakeFilename, DWORD Flags, FOutputDevice* Error )
	{
		...
		FString Tmp = FullPath;
		FILE* File = fcaseopen(appToAnsi(*appUnixPath(FullPath)), "rb");
		if ( UseHomeDir && !File )
		{
			Tmp = FakeFilename;
	...
	FArchive* CreateFileWriter( const TCHAR* FakeFilename, DWORD Flags, FOutputDevice* Error )
	{
		...
		FILE* File = fcaseopen(appToAnsi(*appUnixPath(FullPath)),Mode);
		if( !File )
		{
	...
	void Init(UBOOL Startup)
	{
		...
		if (UseHomeDir)
		{
			wprintf(L"WARNING: using preference directory: %s\n", appToAnsi(*Tmp));
			BaseDir = Tmp;
		}
		else
		{
