...
public:
	...
	void Seek( INT InPos )
	{
		...
		{
			ArIsError = 1;
			Error->Logf(TEXT("Seek outside file bounds: Offset %i / %i"), InPos, Size);
			return;
		}
...
public:
	...
	void Seek( INT InPos )
	{
		...
		{
			ArIsError = 1;
			Error->Logf( TEXT("Error seeking file") );
		}
		Pos = InPos;
	}
	INT Tell()
	...
	UBOOL Close()
	{
		...
		{
			ArIsError = 1;
			Error->Logf( TEXT("Error writing to file") );
		}
		return !ArIsError;
		unguardSlow;
	}
...
public:
	FArchive* CreateFileReader( const TCHAR* Filename, DWORD Flags, FOutputDevice* InError )
	{
		...
		return NewTagged(TEXT("WindowsFileReader"))FArchiveFileReader(Handle, InError, GetFileSize(Handle, NULL));
		unguard;
	}
	FArchive* CreateFileWriter( const TCHAR* Filename, DWORD Flags, FOutputDevice* InError )
	{
		...
		return NewTagged(TEXT("WindowsFileWriter"))FArchiveFileWriter(Handle, InError, Pos);
		unguard;
	}
	...
	UBOOL Move( const TCHAR* Dest, const TCHAR* Src, UBOOL Replace=1, UBOOL EvenIfReadOnly=0, UBOOL Attributes=0 )
	{
		...
		if( !Result )
			debugf( NAME_Warning, TEXT("Error moving file '%ls' to '%ls'"), Src, Dest );
		return Result!=0;
		unguard;
	}
