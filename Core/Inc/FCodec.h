...
public:
	...
	virtual FString GetCompressionName() = 0;
	virtual ~FCodec() {}
};
...
public:
	FString GetCompressionName()
	{
		guard(FCodecBWT::GetCompressionName);
		return TEXT("Burrows-Wheeler");
		unguard;
	}
...
public:
	FString GetCompressionName()
	{
		guard(FCodecRLE::GetCompressionName);
		return TEXT("Run-Length");
		unguard;
	}
...
public:
	FString GetCompressionName()
	{
		guard(FCodecHuffman::GetCompressionName);
		return TEXT("Huffman Coding");
		unguard;
	}
...
public:
	FString GetCompressionName()
	{
		guard(FCodecMTF::GetCompressionName);
		return TEXT("Move-To-Front");
		unguard;
	}
...
private:
	...
	void Code(FArchive& In, FArchive& Out, INT Step, INT First, UBOOL(FCodec::* Func)(FArchive&, FArchive&))
	{
		...
		{
			FString CompName = Codecs(First + Step * i)->GetCompressionName();
			...
			GWarn->Logf(NAME_Heading, TEXT("%s: %f secs"), *CompName, EndTime.GetFloat());
			if (i < Codecs.Num() - 1)
			{
				InData = OutData;
				OutData.Empty();
		...
		GWarn->Logf(NAME_Heading, TEXT("Total: %f secs"), TotalTime);
		unguard;
	}
public:
	FString GetCompressionName()
	{
		guard(FCodecFull::GetCompressionName);
		return TEXT("Full Codec");
		unguard;
	}
