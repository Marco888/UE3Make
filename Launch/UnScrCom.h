...
enum ECompilerPassType
{
	...
	// Constructing objects only.
	PASS_PreProcessStructs,
...
public:
	...
	FArchive& operator<<( class FName& N )
	{
		...
		NAME_INDEX NameIndex = N.GetIndex();
		*this << NameIndex;
		INT Padding = 0;
		*this << Padding;
		return *this;
	...
	{
		//debugf(TEXT("CompileObj %ls %i"), Res->GetFullName(), Tell());
#if 1
	...
	FArchive& operator<<( enum EExprToken E )
	{
		...
		//debugf(TEXT("CompileExpr %ls - %i"), GetExprName(E), Tell());
		BYTE B=E; 
		Serialize(&B,1); 
		return *this;
	}
...
public:
	...
	TMap<UProperty*, UProperty*> HackPropMap;
	UProperty*			HookHack;
	UProperty*			ChildrenHack;
	UProperty*			PropertyLinkHack;
	UProperty*			ConstructorLinkHack;
	...
	UBOOL			CompileScript( TArray<UClass*>& AllClasses, UClass* Class, FMemStack* Mem, UBOOL Booting, ECompilerPassType Pass );
	...
	UProperty*		GetPrivateVarHack(UProperty* InVar, UClass* InClass);
	UProperty*		GetFunctionHookHack();
	...
	UBOOL			CompileFieldExpr( UStruct* Scope, FPropertyBase& RequiredType, FToken Token, FToken& ResultToken, UBOOL IsSelf, UBOOL IsConcrete, FScriptLocation* StartOfExpression, UStruct* RealScope=NULL);
	...
	void			UngetChar();
	UBOOL			IsEOL( TCHAR c );
	void			AddResultText( const TCHAR* Fmt, ... );
	...
	const TCHAR*	GetRestOfLine() const;
...
public:
	...
	void ScriptErrorf(EScriptCompilerErrorLevel ErrorLevel, const TCHAR* Msg, ...);
	void ScriptWarnf(EScriptCompilerWarningLevel WarningLevel, const TCHAR* Msg, ...);
	 ...
	 */
	UBOOL IsDependentOn( UClass* Suspect, UClass* Source, const TArray<UClass*>& AllClasses );
	 ...
	 */
	UBOOL CompileDefaultProperties( const TArray<UClass*>& AllClasses );
	 ...
	 * @return	TRUE if the class defaults were successfully parsed
	 */
	UBOOL CompileClassDefaults( const TArray<UClass*>& AllClasses );
