...
{
	guard(FCommentStrippingFilter::Process);
	TCHAR ch;
	TCHAR matchQuote = ' ';
	UBOOL bQuoted = false, 
		...
		if (bMultiLineComment) 
		{
			if ((ch == TEXT('*')) && SourceBuffer && (SourceBuffer.PeekChar() == TEXT('/'))) 
			{
				...
				if (!bWhitespaceGenerated)
					Result += TEXT(' '); // make sure a multiline comment generates _some_ white space
				bMultiLineComment = false;
			}
...
{
	guard(FMacroProcessingFilter::FMacroProcessingFilter);
	bIsShippingPackage = 0;
	SourcePath = GEditor->EditPackagesInPath;

	PackagePath	= SourcePath * PackageName;
	ClassesPath	= PackagePath * TEXT("Classes");

	EnableOutputFlag.AddItem(true);

	ClassName = fileName.GetFilenameOnly();
	InitSymbolTable();
	unguard;
}
...
{
	guard(FMacroProcessingFilter::InitSymbolTable);
	if ( GlobalSymbols == NULL )
	{
		...
		{
			GlobalSymbols->Set(TEXT("Debug"), TEXT("..."));
		...
		GlobalSymbols->Set(TEXT("ShippingPC"), TEXT("1"));
		GlobalSymbols->Set(FName(TEXT("FINAL_RELEASE")), TEXT("1"));		
		...
		{
			GlobalSymbols->Set(TEXT("Date"), *FString::Printf(TEXT("\"%ls\""), *TimeString.Left(DelimPos)));
			GlobalSymbols->Set(TEXT("Time"), *FString::Printf(TEXT("\"%ls\""), *TimeString.Mid(DelimPos + 1)));
		}
		GlobalSymbols->Set(TEXT("__TW_NETWORKING_"), TEXT("1"));
		GlobalSymbols->Set(TEXT("GameName"), TEXT("\"Killingfloor2\""));
	...
	if (LastPackageProcessed != PackageName)
	{
		...
		FString TestFile = GEditor->EditPackagesInPath * PackageName * TEXT("Globals.uci");
		if(GFileManager->FileSize(*TestFile)>=0)
			ProcessIncludeFile(*TestFile, &GlobalUCIContents);
		else
		{
			TestFile = GEditor->EditPackagesInPath * PackageName + TEXT(".uci");
			ProcessIncludeFile(*TestFile, &GlobalUCIContents);
		}
...
{
	guard(FMacroProcessingFilter::Process);
	TCHAR ch;
...
{
	guard(FMacroProcessingFilter::ExpandMacro);
	...
	if ( macroName == NAME_If )
	{
		if (SourceBuffer.PeekChar() != '(')
		{
			appThrowf(TEXT("Wrong number of parameters for '%s' macro. Expected 1, found %d."), *macroName, parameters.Num());
		}
		else 
		{
			...
			if (SourceBuffer.PeekChar(1) != CALL_MACRO_CHAR)
			{
				...
				if (parameters.Num() > 0)
				{
					ExtraInfo = FString::Printf(TEXT(": `%s(%s)"), *macroName, *parameters(0));
				}
				appThrowf(TEXT("Syntax error in '%s' macro %s"), *macroName, *ExtraInfo);
		}
			...
			else
			{
				...
				if (NestLevel_If >= 1)
				{
					...
					EnableOutputFlag.AddItem(bOutputEnabled);
				}
			}
		}
	}
	...
	else if ( macroName == NAME_Else ) 
	{
		...
		{
			appThrowf(TEXT("Wrong number of parameters for '%s' macro. Expected 0, found %d."), *macroName, parameters.Num());
		}

		if ((NestLevel_If >= 1) && (EnableOutputFlag.Num() > 1)) 
		{
			UBOOL switchValue = !EnableOutputFlag.Pop();
			EnableOutputFlag.AddItem(switchValue);
		}
	}
	...
	else if ( macroName == NAME_Include )
	{
		...
		{
			FString F = GEditor->EditPackagesInPath * PackageName * parameters(0);
			ProcessIncludeFile(*F);
		}
	}
	...
	else if ( macroName == NAME_Undefine )
	{
		...
		else
		{
			...
			{
				debugf(TEXT("Undefining macro '%s' at line %i"),*undefMacroName, SourceBuffer.GetCurrentLineNumber());
				CurrentSymbols.Remove(undefMacroName);
			}
			else
			{
				appThrowf(TEXT("Can't find macro definition to undefine for '%s'"),*undefMacroName);
			}
		}
	}
	else if ( macroName == NAME_Counter || macroName == NAME_GetCounter )
	{
		...
		else 
		{
			...
			// insert value
			SourceBuffer.PushString(FString::Printf(TEXT("%i"),*pCounterValue));
	...
	else if ( macroName == NAME_SetCounter )
	{
		...
		{
			appThrowf(TEXT("Wrong number of parameters for '%s' macro. Expected 2, found %d."), *macroName, parameters.Num());
		}
		else
		{
	...
	{
		SourceBuffer.PushString(FString::Printf(TEXT("%i"),ENGINE_VERSION));
	}
	...
	{
		SourceBuffer.PushString(*SourceBuffer.GetCurrentProvider().Replace(TEXT("\\"), TEXT("\\\\")));
	}
	...
	else if ( Lookup(macroName, definition)) 
	{
		...
		else 
		{
			...
			definition->AddParameterName(*macroName,0);
	...
	{
		GWarn->Logf(TEXT("Unknown macro '%s'."), *macroName);
	}
 ...
 */
void FMacroProcessingFilter::ProcessIncludeFile( const TCHAR* OriginalFilename, FString* FileContent/*=NULL*/ )
{
	guard(FMacroProcessingFilter::ProcessIncludeFile);
	FString fileName = OriginalFilename;
	FString fileContent;
	UBOOL bSuccess = appLoadFileToString(fileContent, *fileName);
	if (!bSuccess && !bInitializingGlobalSymbols)
	{
		static TMap<FString, FString>* ModUciFiles = NULL;
		if (!ModUciFiles)
		{
			guard(CacheGlobalUci);
			ModUciFiles = new TMap<FString, FString>;
			INT i, j;

			TArray<FString> LookDirs;
			new(LookDirs) FString(GEditor->EditPackagesInPath);
			...
			for (i = 0; i < LookDirs.Num(); ++i)
			{
				TArray<FString> Files = GFileManager->FindFiles(*(LookDirs(i) * TEXT("*.uci")), 1, 0);
				for (j = 0; j < Files.Num(); ++j)
				{
					FString FileN = Files(j).GetFilenameOnly();
					if (FileN != TEXT("Globals"))
						ModUciFiles->Set(*Files(j).GetFilenameOnly(), *(LookDirs(i) * Files(j)));
				}
			}
			unguard;
		}
		FString* Result = ModUciFiles->Find(*fileName.GetFilenameOnly());
		if (Result)
		{
			fileName = *Result;
			bSuccess = appLoadFileToString(fileContent, *fileName);
		}
	}
