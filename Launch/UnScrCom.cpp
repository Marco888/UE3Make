...
// Debug logs
#if COMP_DEBUG_LOG
static TCHAR Output[1048];

FDebugLogger* FDebugLogger::DebugLog = NULL;
void FDebugLogger::Serialize(const TCHAR* V, EName Event)
{
	//appSprintf(&Output[Depth], TEXT("%ls (mem %i kb)"), V, (MemUse >> 10));
	appStrcpy(&Output[Depth], V);
	GLog->Log(Output);
}
void FDebugLogger::BeginSubRoutine(const TCHAR* Name)
{
	Logf(TEXT("BEGIN - %ls"), Name);
	Output[Depth++] = '*';
}
void FDebugLogger::EndSubRoutine()
{
	--Depth;
	Log(TEXT("END"));
}

void UEditor::InitDebugger()
{
	static FDebugLogger GLogger;
	FDebugLogger::DebugLog = &GLogger;
}
#else
void UEditor::InitDebugger()
{
...
void FScriptCompiler::ScriptErrorf(EScriptCompilerErrorLevel ErrorLevel, const TCHAR* Fmt, ...)
{
	INT		BufSize = 1024;
	TCHAR* Buffer = NULL;
	INT		Result = -1;
	...
	while (Result == -1)
	{
		...
		{
			GLog->Logf(TEXT("Illegal FScriptCompiler::ScriptErrorf (error: %ls)"), appFromAnsi(strerror(errno)));
			Result = 0;
			break;
		}
		BufSize <<= 1;
...
void FScriptCompiler::ScriptWarnf(EScriptCompilerWarningLevel WarningLevel, const TCHAR* Fmt, ...)
{
	INT		BufSize = 1024;
	TCHAR* Buffer = NULL;
	INT		Result = -1;
	...
	while (Result == -1)
	{
		...
		{
			GLog->Logf(TEXT("Illegal FScriptCompiler::ScriptWarnf (error: %ls)"), appFromAnsi(strerror(errno)));
			Result = 0;
			break;
		}
		BufSize <<= 1;
 ...
 */
UBOOL FScriptCompiler::IsDependentOn( UClass* Suspect, UClass* Source, const TArray<UClass*>& AllClasses )
{
	check( Source );
	check( Suspect );
    ...
    for( INT NameIndex=0; NameIndex<Suspect->DependentOn.Num(); NameIndex++ )
	{
		...
		UClass*	Dependency		= FindClass(*DependencyName);/*FindObject<UClass>( ANY_PACKAGE, *DependencyName.ToString() );*/
	...
	{
		if (AllClasses.FindItemIndex(Suspect) == INDEX_NONE)
		{
...
{
	guard(FClassMetaData::FindFunctionData);
...
INT FScriptWriter::Tell()
{
	return Compiler.TopNode->Script.Num();
}
...
FString FScriptCompiler::GetContext()
{
	if (InDefaultPropContext)
	{
		...
		(
			TEXT("%ls\\%ls\\Classes\\%ls.uc(1) : defaultproperties"),
...
// For error handling.
const TCHAR* FScriptCompiler::GetRestOfLine() const
{
	TCHAR* Result = appStaticString1024();
	const TCHAR* Str = &Input[InputPos];
	INT i;
	for (i = 0; i < 1022 && Str[i] && Str[i] != '\n' && Str[i] != '\r'; ++i)
		Result[i] = Str[i];
	Result[i] = 0;
	return Result;
}
	...
	switch( Token.Type )
	{
		...
		case CPT_String:
		{
			if( appIsPureAnsi(*Token.String) )
			{
				...
				BYTE OutCh;
				for( const TCHAR* Ch=*Token.String; *Ch; Ch++ )
				{
					OutCh = ToAnsi(*Ch);
					Writer << OutCh;
				}
...
TMap<UFunction*,INT> FuncEmitCountMap;
...
void FScriptCompiler::EmitLet( const FPropertyBase& Type, const TCHAR *Tag )
{
	...
	{
		GWarn->Logf(TEXT("Can't assign Const variables"));
	}
	if( Type.ArrayDim > 1 )
	{
...
UBOOL FScriptCompiler::GetToken( FToken& Token, const FPropertyBase* Hint/*=NULL*/, UBOOL NoConsts/*=FALSE*/ )
{
	...
	if( (c>='A' && c<='Z') || (c>='a' && c<='z') || (c=='_') )
	{
		// Alphanumeric token.
		INT Length=0;
		TCHAR* TempIdent = appStaticString1024();
		do
		{
			TempIdent[Length++] = c;
			if( Length >= NAME_SIZE )
		...
		if ( !NoConsts )
		{
			...
			else if( Token.TokenName==NAME_ArrayCount )
			{
				...
				UObject* ObjConst = NULL;
				if (!TypeToken.GetConstObject(UProperty::StaticClass(), ObjConst) || CastChecked<UProperty>(ObjConst)->ArrayDim<=1)
				{
					ScriptWarnf(SCWL_Level4, TEXT("ArrayCount argument is not an array (%ls)"), *TypeToken.TokenName);
				}
				Token.SetConstInt(Max<INT>(ObjConst ? ((UProperty*)ObjConst)->ArrayDim : 1, 1));
				return TRUE;
			}

			else if (Token.TokenName == NAME_NameOf)
			{
#if 1			// Marco: zz lazy fix.
				RequireSymbol(TEXT("("), TEXT("'NameOf'"));
				INT SubDepth = 1;
				FToken NTypeToken;
				while (SubDepth)
				{
					if (MatchSymbol(TEXT("(")))
						...
						--SubDepth;
					else if(!GetToken(NTypeToken))
						ScriptWarnf(SCWL_Level4, TEXT("NameOf argument is missing ')'"));
				}
				if (NTypeToken.TokenType != TOKEN_Identifier || NTypeToken.TokenName == NAME_None)
					ScriptWarnf(SCWL_Level4, TEXT("NameOf argument is not an identifier (%ls)"), *NTypeToken.TokenName);
				Token.SetConstName(NTypeToken.TokenName);
#else
				FToken TypeToken;
				RequireSizeOfParm(TypeToken, TEXT("'NameOf'"));
				UObject* ObjConst = NULL;
				if (!TypeToken.GetConstObject(UField::StaticClass(), ObjConst))
				{
					ScriptWarnf(SCWL_Level4, TEXT("NameOf argument is not an identifier (%ls)"), *TypeToken.TokenName);
				}
				Token.SetConstName(ObjConst ? ObjConst->GetFName() : NAME_None);
#endif
				return TRUE;
			}
			...
			if( Token.TokenName != NAME_None && Hint && Hint->Type == CPT_Byte && Hint->Enum )
			{
				...
				const INT EnumIndex = Hint->Enum->Names.FindItemIndex(Token.TokenName);
			...
			{
				BYTE eIndex = GEditor->FindEnumValue(Token.TokenName);
				if(eIndex<255)
				{
					// Found an enum tag!
					Token.SetConstByte(NULL, eIndex);
					return TRUE;
				}
			}
			...
			if( PeekSymbol(TEXT("'")) )
			{
					...
					&&	MatchSymbol(TEXT("'")) )
				{
					...
					if( Ob == NULL )
					{
						...
						if ( Ob == NULL )
						{
							Ob = UObject::StaticLoadObject( Type, NULL, *Str, NULL, LOAD_Quiet | LOAD_NoVerify | LOAD_NoWarn );
						}
					}
	...
	else if ( !NoConsts && ((c>='0' && c<='9') || ((c=='+' || c=='-') && (p>='0' && p<='9'))) )
	{
		...
		int  Length  = 0;
		int  IsHex   = 0;
		TCHAR* TIdentifier = appStaticString1024();
		do
		{
			if( c==TEXT('.') )
			{
		...
		{
			TCHAR* Start = &Token.Identifier[0];
			TCHAR* End = Start + Token.Identifier.Len();
			if (Hint != NULL && Hint->Type == CPT_Byte)
			{
				Token.SetConstByte(Hint->Enum, static_cast<BYTE>(appStrtoi(Start, &End, 0) & 0xFF));
			}
			else
			{
				Token.SetConstInt(appStrtoi(Start, &End, 0));
			}
		}
	...
	else if( !NoConsts && c=='\'' )
	{
		// Name constant.
		int Length=0;
		TCHAR* TIdent = appStaticString1024();
		c = GetChar();
		...
		// Make constant name.
		Token.SetConstName( FName(TIdent) );
		return TRUE;
	}
	else if( c=='"' )
	{
	...
	else
	{
		...
		// Lookup the token's global name.
		Token.TokenName = FName(TIdent, FNAME_Find);
...
UBOOL FScriptCompiler::GetRawToken( FToken& Token, TCHAR StopChar /* = TCHAR('\n') */ )
{
	...
	static TCHAR Temp[MAX_STRING_CONST_SIZE];
	INT Length=0;
	TCHAR c = GetLeadingChar();
...
{
	guardSlow(MatchSymbol);
	FToken Token;

	if (!GetToken(Token, NULL, 1))
		return 0;

	if (Token.TokenType == TOKEN_Symbol && Token.Identifier == Match)
		return 1;
...
UBOOL FScriptCompiler::PeekSymbol( const TCHAR* Match )
{
	...
	return (Token.TokenType == TOKEN_Symbol && Token.Identifier == Match);
	unguardfSlow((TEXT("(%ls)"), Match));
}
...
UStruct* GetVariableMetaClass(UStruct* Scope, const TCHAR* Name)
{
	UObjectProperty* P = ::FindField<UObjectProperty>(Scope, Name);
	if (!P)
	{
		UClass* C;
		if (Scope->GetClass() == UFunction::StaticClass())
		{
			C = Scope->GetOwnerClass();
			P = ::FindField<UObjectProperty>(C, Name);
		}
		else C = Cast<UClass>(Scope);

		if (!P && C && C->ClassWithin && C->ClassWithin != UObject::StaticClass())
			P = ::FindField<UObjectProperty>(C->ClassWithin, Name);
	}
	if (P)
	{
		if (P->GetClass() == UClassProperty::StaticClass())
			Scope = ((UClassProperty*)P)->MetaClass;
		else Scope = P->PropertyClass;
		if (!Scope)
...
void FScriptCompiler::RequireSizeOfParm( FToken& TypeToken, const TCHAR* Tag )
{
	...
	FToken TempToken;
	if (!GetToken(TempToken) || TempToken.TokenType != TOKEN_Identifier)
	{
PeekFailed:
		ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Bad or missing expression in %s (%ls)"), Tag, GetTokenTypeName(TempToken.TokenType));
		return;
	}

	UStruct* TestScope = TopNodeScope;
	if (MatchSymbol(TEXT(".")))
	{
		UStruct* VarScope = GetVariableMetaClass(TestScope, *TempToken.Identifier);
		if (VarScope)
			TestScope = VarScope;
		else
		{
			UClass* FoundClass = ::FindObject<UClass>(ANY_PACKAGE, *TempToken.Identifier);
			if (!FoundClass)
			{
				ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Failed to find class '%ls' on expression %ls"), *TempToken.Identifier, Tag);
				return;
			}
			if (MatchSymbol(TEXT(".")))
			{
				UState* State = ::FindField<UState>(FoundClass, *TempToken.Identifier);
				if (!State)
				{
					ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Failed to find state '%ls' in %ls on expression %ls"), *TempToken.Identifier, FoundClass->GetFullName(), Tag);
					return;
				}
				TestScope = State;
			}
			else TestScope = FoundClass;
		}
		if (!GetToken(TempToken) || TempToken.TokenType != TOKEN_Identifier)
			goto PeekFailed;
		...
		while (MatchSymbol(TEXT(".")))
		{
			VarScope = GetVariableMetaClass(TestScope, *TempToken.Identifier);
			if (VarScope)
				TestScope = VarScope;
			else ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Failed to find variable '%ls' on %ls in expression %ls"), *TempToken.Identifier, TestScope->GetFullName(), Tag);

			if (!GetToken(TempToken) || TempToken.TokenType != TOKEN_Identifier)
				goto PeekFailed;
		}
	}
	UField* Obj = ::FindField<UField>(TestScope, *TempToken.Identifier);
	if (!Obj && TestScope->GetClass() == UFunction::StaticClass())
	{
		for (UObject* O = TestScope->GetOuter(); (O && !Obj); O = O->GetOuter())
		{
			if(O->IsA(UState::StaticClass()))
				Obj = ::FindField<UField>((UState*)O, *TempToken.Identifier);
		}
	}
	if (!Obj)
	{
		ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Failed to find object '%ls' in %ls on expression %ls"), *TempToken.Identifier, TestScope->GetFullName(), Tag);
		return;
	}

	TypeToken.SetConstObject(Obj);
...
{
	UClass* Result = NULL;
	TCHAR* ClassName = appStaticString1024();
	...
	while( 1 )
	{
		...
		appStrncat(ClassName, *Token.Identifier, 1024);
		if( !MatchSymbol(TEXT(".")) )
		{
			break;
		}
		appStrncat(ClassName, TEXT("."), 1024);
	}
...
)
{
	...
	if( InName!=NAME_None )
	{
		NLogf((TEXT("SEEK '%ls' inside %ls (%ls) within %ls"), *InName, Scope->GetOuter()->GetFullName(), Scope->GetFullName(), OuterContextCount ? Scope->GetOwnerClass()->ClassWithin->GetFullName() : TEXT("None")));
		...
		do
		{
			...
			{
				for( TFieldIterator<UField> It(Scope, bIncludeParents); It; ++It )
				{
					if( It->GetFieldName()==InName )
					{
						NLogf((TEXT("FOUND = %ls"), It->GetFullName()));
...
void FScriptCompiler::CheckObscures( UStruct* Scope, FToken& Token )
{
	...
	while( BaseScope )
	{
		...
		if( Existing )
		{
			ScriptWarnf(SCWL_Level4, TEXT("'%s' obscures '%s' defined in %s class '%s'."), *Token.TokenName, *Token.TokenName, OuterContextCount ? TEXT("outer") : TEXT("base"), Existing->GetOuter()->GetName());
		}
...
UBOOL FScriptCompiler::HasMemberProperties( const UStruct* Scope )
{
	...
	checkSlow(Scope);
	TFieldIterator<UProperty> It(Scope);
...
UStruct* FScriptCompiler::GetSuperScope( UStruct* CurrentScope, const FName& SearchName )
{
	...
	if (SuperScope->GetClass() == UFunction::StaticClass())
		SuperScope = SuperScope->GetOwnerClass();
...
UEnum* FScriptCompiler::CompileEnum( UStruct* Scope, UBOOL& NeedSemicolon )
{
	...
	{
		ScriptErrorf(SCEL_Unknown/*SCEL_Parse*/,  TEXT("enum: '%s' already defined here"), *EnumToken.TokenName );
	}
	...
	UEnum* Enum = FindObject<UEnum>(Scope, *EnumToken.Identifier, 1);
	if (!Enum)
		Enum = new(Scope, *EnumToken.Identifier, RF_Public) UEnum();
	...
	while( GetIdentifier(TagToken) )
	{
		...
		INT iFound;
		FName NewTag(*TagToken.Identifier, FNAME_Add);
		...
		else if ( TagToken.MetaData.Num() > 0 )
		{
			...
			{
				FString KeyString = TokenString + TEXT(".") + FString(*MetaDataIt.Key());
	...
	if ( !Enum->SetEnums( EnumNames ) )
	{
		...
		ScriptErrorf(SCEL_Restricted, TEXT("Illegal enumeration tag specified.  Conflicts with auto-generated tag '%s'"), *MaxEnumItem);
		///return NULL;
	}
...
{
#if 0 // Marco: PFFT we dont need this here!
	...
	if ( bContainsEOLComment )
	{
		...
		if ( !bReparsingClass && (VarProperty.PropertyFlags&CPF_Edit) != 0 && !VarProperty.MetaData.Find(ToolTipName) )
		{
...
UScriptStruct* FScriptCompiler::CompileStruct( UStruct* Scope, UBOOL& NeedSemicolon )
{
	...
	if( Existing && Existing->GetOuter()==Scope && !bReparsingClass )
	{
		ScriptErrorf(SCEL_Unknown,  TEXT("struct: '%s' already defined here"), *StructToken.TokenName );
	}
	else
	{
	...
	{
		// Create.
		Struct = FindObject<UScriptStruct>(Scope, *StructToken.Identifier, 1);
		if(!Struct)
			Struct = new( Scope, *StructToken.Identifier, RF_Public )UScriptStruct(BaseStruct);
		else
		{
			Struct->SuperStruct = BaseStruct;
			Struct->PropertiesSize = BaseStruct ? BaseStruct->GetPropertiesSize() : 0;
		}
	...
	do
	{
				...
				do
				{
					...
					if( GetIdentifier( CategoryToken, 1 ) )
					{
						if( CategoryPath.Len() > 0 )
						{
							...
							CategoryPath += CategoryDelimiterChar;
						}
						CategoryPath += CategoryToken.Identifier;
					}
					else
					{
						EdCategory = FName( *StructToken.Identifier, FNAME_Find );
						break;
					}
				}
				while( MatchSymbol(TEXT(",")) );
		...
		else if ( Token.Matches( TEXT("structcpptext") ) )
		{
			...
			if (CppText.Len() && !bReparsingClass)
			{
				...
				{
					Struct->CppText = new(Struct, TEXT("CppText"), RF_Transient) UTextBuffer(*CppText);
				}
			}
		}
	...
	if ( !bReparsingClass )
	{
		...
		if ( Struct->GetPropertiesSize() < 1 && (Struct->StructFlags&STRUCT_Native) != 0)
		{
			ScriptWarnf(SCWL_Level4, TEXT("%s must contain at least %d bytes (currently only contains %d)"), Struct->GetName(), 1, Struct->GetPropertiesSize());
		}
	}
	return Struct;
}
...
void FScriptCompiler::CompileConst( UStruct* Scope, UBOOL& NeedSemicolon )
{
	...
	{
		ScriptErrorf(SCEL_Unknown/*SCEL_Parse*/,  TEXT("const %s: Missing value"), *ConstName );
	}
	// Format constant.
	...
	if( ValueToken.TokenType != TOKEN_Const )
	{
		...
		{
			ScriptErrorf(SCEL_Parse,  TEXT("const %s: Value is not constant"), *ConstName );
		}
	}
	else
	{
...
{
	guard(::CompileFieldExpr);
	NBegin(CompileFieldExpr);
	...
	NLogf((TEXT("Token=%ls"), Token.Desc()));
		...
		else
		{
			...
			if (!SuperScope)
			{
				...
				if (FindObject<UStruct>(ANY_PACKAGE, *SuperToken.Identifier))
					ScriptErrorf(SCEL_Restricted, TEXT("'Super(classname.statename)': '%s' does not extend '%s'"), Scope->GetOwnerClass()->GetName(), *SuperToken.Identifier);
				else ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Bad class name '%s'"), *SuperToken.Identifier);
				return 0;
			}
			...
			if (MatchSymbol(TEXT(".")))
			{
				if (!GetIdentifier(SuperToken))
				{
					ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Missing state name"));
				}

				UState* StateScope = ::FindField<UState>(SuperScope, *SuperToken.Identifier);
				if (!StateScope)
				{
					ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Bad state name '%s' in %ls"), *SuperToken.Identifier, SuperScope->GetFullName());
				}
				Scope = StateScope;
			}
			else
	...
	else if (Token.Matches(NAME_Context))
	{
		RequireSymbol(TEXT("("), TEXT("'context'"));
		// get the desired super class/state name
		FToken ContextToken;
		if (!GetIdentifier(ContextToken))
		{
			ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Missing class name"));
		}
		// find the current state/class

		UStruct* ContextScope = FindObject<UClass>(ANY_PACKAGE, *ContextToken.Identifier, 1);
		if(ContextScope && MatchSymbol(TEXT(".")))
		{
			if (!GetIdentifier(ContextToken))
			{
				ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Missing state name"));
			}
			ContextScope = ::FindField<UState>(ContextScope, *ContextToken.Identifier);
		}
		if (!ContextScope)
		{
			ScriptErrorf(SCEL_Restricted, TEXT("'Context(class.state)': Couldn't find class '%s'"), *ContextToken.Identifier);
		}
		Scope = ContextScope;
		RequireSymbol(TEXT(")"), TEXT("'Context(class.state)'"));
		RequireSymbol(TEXT("."), TEXT("'Context(class.state)'"));
		ForceFinal = 1;
		GetToken(Token);
	}
	...
	// Handle field type.
	UField* Field = Field = FindField(Scope, *Token.Identifier, TRUE, FieldClass, NULL, IsSelf ? &OuterContextCount : NULL);
	...
	if (!Field)
	{
		if (Token.Identifier == TEXT("Log"))
			Field = FindObject<UFunction>(UObject::StaticClass(), TEXT("LogInternal"));
		else if (Token.Identifier == TEXT("Warn"))
			Field = FindObject<UFunction>(UObject::StaticClass(), TEXT("WarnInternal"));
	}
	if (!Field && RealScope && RealScope != Scope)
	{
		Field = FindField(RealScope, *Token.Identifier, TRUE, FieldClass, NULL, IsSelf ? &OuterContextCount : NULL);
	}

	// Try to see if this is accessing a function pointer through classname.
	if (!Field && PeekSymbol(TEXT(".")))
	{
		UClass* ClFl = FindObject<UClass>(ANY_PACKAGE, *Token.Identifier, 1);
		if (ClFl)
		{
			RequireSymbol(TEXT("."), TEXT("'ClassFunction'"));
			FToken TestToken;
			GetToken(TestToken);
			UField* FF = FindField(ClFl, *TestToken.Identifier, FALSE, UField::StaticClass());
			if (FF && FF->IsA(UState::StaticClass()) && MatchSymbol(TEXT(".")))
			{
				GetToken(TestToken);
				FF = FindField((UState*)FF, *TestToken.Identifier, FALSE, UField::StaticClass());
			}
			if (FF && FF->IsA(UFunction::StaticClass()))
			{
				Field = FF;
				Token.Clone(TestToken);
			}
			else GWarn->Logf(TEXT("Unable to find function/state %ls in %ls"), *TestToken.Identifier, ClFl->GetFullName());
		}
	}
	NLogf((TEXT("GetField %ls"), Field->GetFullName()));
	...
	else if (Field && Cast<UProperty>(Field))
	{
		...
		if( !Property->HasAnyFlags(RF_Public) && OwnerClass->GetOuter()!=Class->GetOuter() )
		{
			GWarn->Logf(TEXT("Can't access private variable '%s' in '%s'"), Property->GetName(), OwnerClass->GetName());
			Property = GetPrivateVarHack(Property, OwnerClass);
		}

		// cannot access properties in native-only class
		if (OwnerClass && OwnerClass->HasAnyClassFlags(CLASS_NativeOnly))
		{
			GWarn->Logf(TEXT("Can't access variable '%s' in native-only class '%s'"), Property->GetName(), OwnerClass->GetName() );
		}
		...
		if ( Property->HasAnyFlags(RF_Protected) )
		{
			...
			{
				GWarn->Logf(TEXT("Can't access protected variable '%s' in '%s'"), Property->GetName(), OwnerClass->GetName());
			}
		}
		...
		{
			GWarn->Logf(TEXT("You can only access default values of variables here"));
		}
		...
		if ( (RealScope ? RealScope : Scope)->GetClass() == UFunction::StaticClass() && bIsLocalProperty )
		{
			if ( LocalProperties )
			{
	...
	else if ( RequiredType.Type == CPT_Delegate )
	{
		if ( FieldFunction != NULL )
		{
			...
			if (RequiredType.Function != NULL)
			{
				...
				{
					ScriptErrorf(SCEL_Restricted,  TEXT("'%s' mismatches delegate '%s'"), *FieldFunction->FriendlyName, *RequiredType.Function->FriendlyName );
				}
		...
		else
		{
			if ( Field == NULL )
			{
				...
				{
					ScriptErrorf(SCEL_Unknown,  TEXT("Delegate assignment failed (%s): Invalid or unknown function '%s'"), *RequiredType.Function->FriendlyName, *Token.Identifier );
				}
			...
			{
				ScriptErrorf(SCEL_Unknown,  TEXT("Delegate assignment failed (%s): '%s' is not a function"), *RequiredType.Function->FriendlyName, Field->GetFullName() );
			}
			return 1;
		}
	}
	else if( FieldFunction != NULL )
	{
		...
		if ( MatchSymbol(TEXT("(")) )
		{
			if (FieldFunction->CustomScope && FieldFunction->CustomScope->GetClass() == UFunction::StaticClass())
				ScriptErrorf(SCEL_Unknown, TEXT("Cannot call '%s' which is context to '%ls'"), FieldFunction->GetFullName(), FieldFunction->CustomScope->GetFullName());

			UClass* ScopedClass = TopNodeScope->GetOwnerClass();
			GotAffector = 1;
			...
			if( FieldFunction->HasAnyFunctionFlags(FUNC_Private) && FieldFunction->GetOwnerClass()!= ScopedClass)
			{
				debugf(TEXT("Can't access private function '%s' in '%s'"), FieldFunction->GetName(), FieldFunction->GetOwnerClass()->GetName() );
			}
			if(	FieldFunction->HasAnyFunctionFlags(FUNC_Protected) )
			{
				...
				if( !ScopedClass->IsChildOf( FieldFunction->GetOwnerClass() ) )													// if we are not a subclass of the function's class.
				{
					if( FieldFunction->GetOwnerClass()->IsChildOf(ScopedClass) )												// if we are a superclass of the function's class.
					{
						INT OuterContextCnt = FALSE;
						
						// Fix ARL: Use correct scope to account for functions in states.
						UFunction* ScopedFunction = Cast<UFunction>(FindField(ScopedClass, FieldFunction->GetName(), TRUE, UFunction::StaticClass(), TEXT("'function'"), &OuterContextCnt));
						if( ScopedFunction==NULL )																			// if the function is not in the superclass's scope.
						{
							debugf(TEXT("Can't access protected function '%s' in '%s' (not in scope)"), FieldFunction->GetName(), FieldFunction->GetOwnerClass()->GetName() );
						}
						else if( (ScopedFunction->FunctionFlags & FUNC_Private) && ScopedFunction->GetOwnerClass()!= ScopedClass)	// if a function with the same name is in scope but is private.
						{
							debugf(TEXT("Can't access private function '%s' in '%s' (%s::%s is not in scope)"),
								ScopedFunction->GetName(), ScopedFunction->GetOwnerClass()->GetName(), FieldFunction->GetOwnerClass()->GetName(), FieldFunction->GetName() );
						}
					}
					else
					{
						debugf(TEXT("Can't access protected function '%s' in '%s'"), FieldFunction->GetName(), FieldFunction->GetOwnerClass()->GetName() );
					}
				}
			}
			...
			{
				debugf(TEXT("Can't call instance functions from within static functions") );
			}
			if( Static && !FieldFunction->HasAnyFunctionFlags(FUNC_Static) )
			{
				debugf(TEXT("Function '%s' is not static"), FieldFunction->GetName() );
			}
			...
			&&	(FieldFunction->NumParms>=2) )
			{
				...
				UObjectProperty* A = Cast<UObjectProperty>(*It);
				++It;
		...
		else if (RequiredType.bIsLValue)
		{
			if (FieldFunction->iNative)
				ScriptErrorf(SCEL_Expression, TEXT("Impossible to hook numbered native function (%ls)!"), FieldFunction->GetFullName());
			ResultType.PropertyFlags &= ~CPF_OutParm;
			ResultType.SetConstObject(FieldFunction);
			return 1;
		}
		else
		{
	...
	else
	{
		...
		{
			BYTE Result = GEditor->FindEnumValue(Token.TokenName);

			if (Result < 255)
			{
				ResultType.SetConstByte(NULL, Result);
				ResultType.PropertyFlags &= ~CPF_OutParm;
				EmitConstant(ResultType);
			...
			UConst* Const = FindObject<UConst>(ANY_PACKAGE, *Token.TokenName, 1);
			if (Const)
			{
				// Named constant.
				// read the const value into a token
...
static FName GetUniqueVarName(UObject* Outer)
{
	if (GEditor->bShouldObfuscate)
		return UEditor::GetObscNameUnique(Outer);

	static INT iVarCount = 0;
	while (1)
	{
		FName Result(*FString::Printf(TEXT("H%X"), iVarCount++));
		if (UObject::StaticFindObject(UObject::StaticClass(), Outer, *Result) == NULL)
			return Result;
	}
}

static UProperty* ConstructPropertyClone(UProperty* Property, UObject* NewPck, FScriptCompiler* Compiler)
{
	UProperty* NewProp = ConstructObject<UProperty>(Property->GetClass(), NewPck, GetUniqueVarName(NewPck), RF_Public);
	Compiler->HackPropMap.Set(Property, NewProp);

	// Make clone of the property.
	NewProp->ArrayDim = Property->ArrayDim;
	NewProp->ElementSize = Property->ElementSize;
	NewProp->PropertyFlags = CPF_Transient;
	NewProp->Offset = Property->Offset;
	NewProp->LinkAlignment = Property->LinkAlignment;

	if (Property->IsA(UBoolProperty::StaticClass()))
		((UBoolProperty*)NewProp)->BitMask = ((UBoolProperty*)Property)->BitMask;
	else if (Property->IsA(UObjectProperty::StaticClass()))
	{
		((UObjectProperty*)NewProp)->PropertyClass = ((UObjectProperty*)Property)->PropertyClass;
		if (Property->IsA(UClassProperty::StaticClass()))
			((UClassProperty*)NewProp)->MetaClass = ((UClassProperty*)Property)->MetaClass;
	}
	else if (Property->IsA(UInterfaceProperty::StaticClass()))
		((UInterfaceProperty*)NewProp)->InterfaceClass = ((UInterfaceProperty*)Property)->InterfaceClass;
	else if (Property->IsA(UArrayProperty::StaticClass()))
		((UArrayProperty*)NewProp)->Inner = ConstructPropertyClone(((UArrayProperty*)Property)->Inner, NewProp, Compiler);
	else if (Property->IsA(UStructProperty::StaticClass()))
		((UStructProperty*)NewProp)->Struct = ((UStructProperty*)Property)->Struct;
	return NewProp;
}

UProperty* FScriptCompiler::GetPrivateVarHack(UProperty* InVar, UClass* InClass)
{
	UProperty* Result = HackPropMap.FindRef(InVar);
	if (!Result)
	{
		UClass* HackObj = new(Class->GetOuter(), GetUniqueVarName(Class->GetOuter()), RF_Public) UClass(InClass->GetSuperClass());
		HackObj->ClassFlags = (CLASS_Abstract | CLASS_Compiled | CLASS_Parsed | CLASS_NoScriptProps);

		UProperty* PrevProp = NULL;

		for (TFieldIterator<UProperty> It(InClass, 0); It; ++It)
		{
			UProperty* NewProp = ConstructPropertyClone(*It, HackObj, this);
			if (PrevProp)
				PrevProp->Next = NewProp;
			else HackObj->Children = NewProp;
			PrevProp = NewProp;
		}

		FArchive DummyAr;
		HackObj->Link(DummyAr, 1);

		DWORD PreviousHackFlags = GUglyHackFlags;
		GUglyHackFlags |= HACK_DisableSubobjectInstancing;

		HackObj->GetDefaultObject(TRUE);
		GUglyHackFlags = PreviousHackFlags;

		Result = HackPropMap.FindRef(InVar);
		if (!Result)
			ScriptErrorf(SCEL_Unknown, TEXT("Unable to linkup private variable to private hack for %ls"), InVar->GetFullName());
	}
	return Result;
}

UProperty* FScriptCompiler::GetFunctionHookHack()
{
	guard(FScriptCompiler::GetFunctionHookHack);
	if (!HookHack)
	{
		UClass* HackObj = new(Class->GetOuter(), GetUniqueVarName(Class->GetOuter()), RF_Public) UClass(UObject::StaticClass());
		HackObj->ClassFlags = (CLASS_Abstract | CLASS_Compiled | CLASS_Parsed | CLASS_NoScriptProps);
		UProperty* LastProp;

		{
			UByteProperty* DummyBytes = new(HackObj, GetUniqueVarName(HackObj), 0)UByteProperty();
#if WITH_FUNC_PARMS
			DummyBytes->ArrayDim = GEditor->iFunctionOffset - sizeof(INT) - sizeof(ScriptPointerType);
#else
			DummyBytes->ArrayDim = GEditor->iFunctionOffset;
#endif
			HackObj->Children = DummyBytes;
			LastProp = DummyBytes;
		}
#if WITH_FUNC_PARMS
		{
			UObjectProperty* Childrens = new(HackObj, GetUniqueVarName(), 0)UObjectProperty();
			Childrens->PropertyClass = UField::StaticClass();
			Childrens->ArrayDim = 1;
			LastProp->Next = Childrens;
			LastProp = Childrens;
			ChildrenHack = Childrens;
		}
		{
			UByteProperty* DummyBytes = new(HackObj, GetUniqueVarName(), 0)UByteProperty();
			DummyBytes->ArrayDim = sizeof(INT);
			LastProp->Next = DummyBytes;
			LastProp = DummyBytes;
		}
#endif
		{
			UArrayProperty* HackAr = new(HackObj, GetUniqueVarName(HackObj), 0)UArrayProperty();
			HackAr->ArrayDim = 1;
			UByteProperty* InnerBytes = new(HackAr, GetUniqueVarName(HackAr), 0)UByteProperty();
			InnerBytes->ArrayDim = 1;
			HackAr->Inner = InnerBytes;
			LastProp->Next = HackAr;
			HookHack = HackAr;
			LastProp = HackAr;
		}
#if WITH_FUNC_PARMS
		{
			UByteProperty* DummyBytes = new(HackObj, GetUniqueVarName(), 0)UByteProperty();
			DummyBytes->ArrayDim = sizeof(INT) * 3 + sizeof(ScriptPointerType);
			LastProp->Next = DummyBytes;
			LastProp = DummyBytes;
		}
		{
			UObjectProperty* PropertyLink = new(HackObj, GetUniqueVarName(), 0)UObjectProperty();
			PropertyLink->PropertyClass = UField::StaticClass();
			PropertyLink->ArrayDim = 1;
			LastProp->Next = PropertyLink;
			LastProp = PropertyLink;
			PropertyLinkHack = PropertyLink;
		}
		{
			UByteProperty* DummyBytes = new(HackObj, GetUniqueVarName(), 0)UByteProperty();
			DummyBytes->ArrayDim = sizeof(ScriptPointerType);
			LastProp->Next = DummyBytes;
			LastProp = DummyBytes;
		}
		{
			UObjectProperty* ConstructorLink = new(HackObj, GetUniqueVarName(), 0)UObjectProperty();
			ConstructorLink->PropertyClass = UField::StaticClass();
			ConstructorLink->ArrayDim = 1;
			LastProp->Next = ConstructorLink;
			LastProp = ConstructorLink;
			ConstructorLinkHack = ConstructorLink;
		}
#endif
...
{
	guard(::CompileDynamicCast);
	NBegin(CompileDynamicCast);
	...
	NLogf((TEXT("DestClass %ls"), DestClass->GetFullName()));
	if( !DestClass )
	{
		UEnum* DestEnum = FindObject<UEnum>( ANY_PACKAGE, *Token.Identifier );
		NLogf((TEXT("DestEnum %ls"), DestEnum->GetFullName()));
		if (DestEnum)
		{
	...
	if( DestClass==UClass::StaticClass() )
	{
		if( MatchSymbol(TEXT("<")) )
		{
			...
			NLogf((TEXT("GetMetaToken %ls"), MetaClassToken.Desc()));
...
)
{
	...
	{
		NLogf((TEXT("GetToken %ls"), Token.Desc()));
		...
		&&	MatchSymbol(TEXT("(")) )
		{
			...
			if ( TopNest->NestType == NEST_Switch && ResultToken == &TopNest->SwitchType )
			{
				...
				||	ToType.Type == CPT_Struct)
				{
					...
					ScriptErrorf(SCEL_Restricted, TEXT("Can't switch on dynamic type conversion for '%s': %s"), *Token.Identifier, *FromType.TokenName);
					return 0;
				}
			}
			...
			{
				ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/,  TEXT("'%s' conversion: Bad or missing expression"), *Token.TokenName );
				///return FALSE;
			}
			else if (FromType.ArrayDim != 1)
			{
				ScriptErrorf(SCEL_Unknown, TEXT("'%s' conversion: Expression is an array"), *Token.TokenName);
			}
			...
			else if( ToType.Type == FromType.Type && (ToType.Type != CPT_Struct || ToType.Struct == FromType.Struct) )
			{
				...
				ScriptErrorf(SCEL_Restricted,  TEXT("No need to cast '%s' to itself"), *DestType);
			}
			else
			{
				FName SourceType = FromType.Type == CPT_Struct ? FromType.Struct->GetFName() : GetPropertyName(FromType.Type);
				FName DestType = ToType.Type == CPT_Struct ? ToType.Struct->GetFName() : GetPropertyName(ToType.Type);
				ScriptErrorf(SCEL_Restricted,  TEXT("Can't convert '%s' to '%s'"), *SourceType, *DestType);
			}
		...
		else if( Token.TokenName==NAME_Self )
		{
			...
			Writer << EX_Self;
			if(TopNodeScope!=TopNode)
				Token = FToken(FPropertyBase(TopNodeScope->GetOwnerClass()));
			else Token = FToken(FPropertyBase( Class ));
			Token.TokenName = NAME_Self;
		}
		else if (Token.TokenName == NAME_TypeCast)
		{
			FPropertyBase CastProperty(CPT_None);
			EObjectFlags ObjectFlags = 0;

			const TCHAR TagType[] = TEXT("'TypeCast'");
			RequireSymbol(TEXT("<"), TagType);
			// Get var type.
			if (!GetVarType(TopNode, CastProperty, ObjectFlags, 0, TagType))
				ScriptErrorf(SCEL_Restricted, TEXT("Missing typecasting property type."));
			RequireSymbol(TEXT(">"), TagType);
			RequireSymbol(TEXT("("), TagType);
			if (!CompileExpr(FPropertyBase(CPT_None), TagType, &Token))
				ScriptErrorf(SCEL_Restricted, TEXT("Bad or missing typecasting input variable."));
			RequireSymbol(TEXT(")"), TagType);

			Token.Type = CastProperty.Type;
			Token.Enum = CastProperty.Enum;
			Token.BitMask = CastProperty.BitMask;
			Token.MetaClass = CastProperty.MetaClass;
			Token.PropertyClass = CastProperty.PropertyClass;
			//appThrowf( TEXT("TypeCast was ok: %ls,%ls"), GetPropName(CastProperty.Type), *TypeToken.TokenName );
		}
		...
		else if( CompileFieldExpr(TopNodeScope, RequiredType, Token, Token, 1, (TopNest->Allow&ALLOW_Instance)!=0, &StartOfExpression, TopNode ) )
		{
			...
			if ( TokenList )
			{
				...
				if ( LastToken.Matches(NAME_Super) )
				{
					...
					if ( PeekSymbol(TEXT("(")) )
					{
						...
						if ( GetIdentifier(SuperToken) ) 
						{
							UStruct* SuperClass = GetSuperScope(TopNodeScope,SuperToken.TokenName);
							if ( SuperClass != NULL )
							{
							...
							else
							{
								...
								if (FindObject<UStruct>(ANY_PACKAGE, *SuperToken.Identifier))
								{
									ScriptErrorf(SCEL_Restricted, TEXT("'Super(name)': '%s' does not extend '%s'"), TopNodeScope->GetOuter()->GetName(), *SuperToken.Identifier);
								}
								else
								{
	...
	for( ; ; )
	{
		...
		{
			Prop = Cast<UProperty>(FindField(Scope, *TokenIdentifier, TRUE, UProperty::StaticClass(), NULL, &OuterContextCount));
		}

		if (Prop == NULL && Scope != TopNode)
		{
			// and second in TopNode (for locals, etc)
			Prop = Cast<UProperty>(FindField(TopNode, *TokenIdentifier, TRUE, UProperty::StaticClass(), NULL, &OuterContextCount));
		}

		if ( Prop != NULL )
		{
			NLogf((TEXT("Property %ls"), Prop->GetFullName()));
		...
		if ( MatchSymbol(TEXT(".")) )
		{
			...
			if( Token.IsDynamicArray() )
			{
				if( MatchIdentifier(NAME_Length) )
				{
					...
					{
						ScriptErrorf(SCEL_Restricted, TEXT("Illegal to use operator %s to modify the length of a dynamic array"), *HintType->TokenName);
					}
				...
				else if (MatchIdentifier(NAME_Add))
				{
					...
					{
						GWarn->Logf(TEXT("Attempting to call 'Add' on const dynamic array [%s]"), Prop != NULL ? Prop->GetName() : TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
				...
				else if (MatchIdentifier(NAME_AddItem))
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'AddItem' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
				...
				else if( MatchIdentifier(NAME_Insert) )
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'Insert' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
				...
				else if (MatchIdentifier(NAME_InsertItem))
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'InsertItem' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
				...
				else if( MatchIdentifier(NAME_Remove) )
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'Remove' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
				...
				else if (MatchIdentifier(NAME_RemoveItem))
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'RemoveItem' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
				...
				else if ( MatchIdentifier(NAME_Find) )
				{
					...
					if (InnerStructProp != NULL)
					{
						...
						UProperty* StructProp = Cast<UProperty>(FindField(Struct, *PropToken.Identifier, TRUE, UProperty::StaticClass()));
						if ( StructProp == NULL )
							ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/,  TEXT("Unknown member '%s' in struct '%s'"), *PropToken.Identifier, Struct->GetPathName() );
				...
				else if (MatchIdentifier(NAME_Sort))
				{
					...
					if (Token.PropertyFlags & CPF_Const)
					{
						GWarn->Logf(TEXT("Attempting to call 'Sort' on const dynamic array [%s]"),Prop!=NULL?Prop->GetName():TEXT("Unknown"));
					}
					FScriptLocation HighRetry;
			...
			else if( Token.Type==CPT_Struct )
			{
				...
				UProperty* Member = ::FindField<UProperty>(Token.Struct, *Tag.TokenName);
				if( !Member )
				{
					ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/,  TEXT("Unknown member '%s' in struct '%s'"), *Tag.Identifier, Token.Struct->GetPathName() );
				}
			...
			else if( Token.IsObject() )
			{
				...
				{
					checkf(Token.MetaClass, TEXT("Unresolved class for 'class' expression - File:%s  Line:%i"), Class->GetName(), InputLine);
...
//@todo ronp - should we put code here for emitting Outer context?
				...
				NLogf((TEXT("GetContextToken %ls"), ContextToken.Desc()));
...
Test:
	...
	{
		NLogf((TEXT("GetOperToken %ls"), OperToken.Desc()));
		if( OperToken.TokenName != NAME_None )
		{
			...
			FString OperTokenString = FString(*OperToken.TokenName).Replace(TEXT("%"), TEXT("[Modulo]"));
						...
						else if ( bValidDelegateComparison )
						{
							...
							&&	Token.Function != NewResultType.Function )
							{
								ScriptErrorf(SCEL_Restricted, TEXT("Mismatched delegates in %s; expected '%s', got '%s'"), *OperTokenString, 
									*Token.Function->FriendlyName, *NewResultType.Function->FriendlyName);
							}
							FScriptLocation HighRetry;
							...
							if ( NewResultType.TokenFunction != NULL )
							{
								...
								{
									ScriptErrorf(SCEL_Restricted,  TEXT("'%s' mismatches delegate '%s' in %s"), *NormalFunction->FriendlyName, *DelegateFunction->FriendlyName, *OperTokenString );
								}
				...
				else
				{
					...
					if ( OperParm1 != NULL && OperParm2 != NULL )
					{
						...
						&&	(!OperParm1->HasAnyPropertyFlags(CPF_CoerceParm) || !OperParm2->HasAnyPropertyFlags(CPF_CoerceParm)) )
						{
							...
							{
								const INT NumEnumValues = Token.Enum->Names.Num();

								// and they have the same number of values
								if ( NumEnumValues == NewResultType.Enum->Names.Num() )
								{
									bMirroredEnums = TRUE;
									...
									{
										const FName LeftEnumValue = Token.Enum->Names(EnumValueIndex);
										const FName RightEnumValue = NewResultType.Enum->Names(EnumValueIndex);
							...
							if ( !bMirroredEnums )
							{
								ScriptErrorf(SCEL_Restricted, TEXT("Operator '%s': Attempting to perform comparison between variables of different enum types with '%s' (%s) and '%s' (%s)"),
									*OperTokenString, *Token.TokenName, Token.Enum->GetPathName(), *NewResultType.TokenName, NewResultType.Enum->GetPathName());
							}
						}
					}
					...
					if( BestOperLink->NumParms==3 || IsPreOperator )
					{
						...
						if( OperParm2->PropertyFlags & CPF_SkipParm )
						{
							...
							FScriptLocation HighRetry;
							CodeSkipSizeType wOffset = 1 + HighRetry.CodeTop - MidRetry.CodeTop;
							Writer << EX_Skip;
							Writer << wOffset;
	...
	if( Token.Type==CPT_None && RequiredType.Type!=CPT_None )
	{
		...
		if( !(RequiredType.PropertyFlags & CPF_OptionalParm) && ErrorTag )
		{
			...
			{
				ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Bad or missing expression for token: %s, in %s"), *Token.TokenName, ErrorTag);
			}
		}
		if( ResultToken )
		{
	...
	unguardf((TEXT("(Line=%i %ls)"), iCompileLine, GetRestOfLine()));
}
...
void FNestInfo::Init()
{
	ChainJumpPlaceholderLocation = INDEX_NONE;
	SwitchType = FToken();
	JumpPlaceholderList = NULL;
	StructModificationByteList = NULL;
	LabelList = NULL;
	NestFlags = 0;
...
void FScriptCompiler::PushNest( ENestType NestType, FName ThisName, UStruct* InNode )
{
	...
	if( Pass==PASS_Parse && (NestType==NEST_State || NestType==NEST_Function) )
	{
		for (TFieldIterator<UField> It(TopNode, FALSE); It; ++It)
		{
			if (It->GetFName() == ThisName)
			{
				...
				{
					ScriptErrorf(SCEL_Restricted, TEXT("'%s' conflicts with '%s'"), *ThisName, It->GetFullName());
				}
			}
		}
	}
	...
	TopNodeScope = TopNode;
	TopNodeFunc = TopNode;
	if (TopNode && TopNode->GetClass() == UFunction::StaticClass() && ((UFunction*)TopNode)->CustomScope)
	{
		TopNodeScope = ((UFunction*)TopNode)->CustomScope;
		if (TopNodeScope->IsA(UFunction::StaticClass()))
			TopNodeFunc = TopNodeScope;
	}
	unguard;
}
...
void FScriptCompiler::PopNest( ENestType NestType, const TCHAR* Descr )
{
	...
	if( Pass == PASS_Parse )
	{
				...
				for ( TArray<FImplementedInterface>::TIterator It(TopClass->Interfaces); It; ++It )
				{
					...
					for( UClass* Interface = It->Class; Interface; Interface = Interface->GetSuperClass() )
					{
						...
						if ( !TopNode->IsChildOf( Interface ) )
						{
							...
							for( TFieldIterator<UFunction> InterfaceFunction(Interface); InterfaceFunction; ++InterfaceFunction )
							{
		...
		else if ( NestType == NEST_Interface )
		{
			...
			{
				TMap<FName,UFunction*> DelegateCache;
				FixupDelegateProperties(TopNode, Cast<UClass>(TopNode), DelegateCache);
			}
		}
		else
		{
			...
			if( !(TopFunction->FunctionFlags & FUNC_Native) )
			{
 				...
 				if( LocalProperties )
				{
					...
					for( TFieldIterator<UProperty> It(TopNode); It; ++It )
					{
						...
						if ((property->PropertyFlags & CPF_Parm) == 0 && (TopNode == TopNodeFunc))
						{
							FLocalProperty* CurLocal = LocalProperties->GetLocalProperty(property);
...
void FScriptCompiler::ReplacePlaceholderValues( FNestInfo* Nest )
{
	...
	for( FJumpTargetPlaceholder* Fixup = Nest->JumpPlaceholderList; Fixup!=NULL; Fixup=Fixup->Next )
	{
		...
		if( Fixup->Type == FIXUP_Label )
		{
			...
			if( LabelRecord == NULL )
			{
				ScriptErrorf(SCEL_Unknown,  TEXT("Label '%s' not found in this block of code"), *Fixup->Name );
			}
		}
		else
		{
		...
		if ( Property != NULL )
		{
			...
			if ( DelegateProperty != NULL )
			{
				...
				if ( bDelegateVariable )
				{
					...
					{
						FString NameOfDelegateFunction = *DelegatePropertyToken->Token.DelegateName;
						if ( NameOfDelegateFunction.InStr(TEXT(".")) == INDEX_NONE )
						{
							...
							SourceDelegateFunction = Cast<UFunction>(FindField(OwnerClass, *NameOfDelegateFunction, TRUE, UFunction::StaticClass(), NULL, NULL));
							if ( SourceDelegateFunction == NULL )
							{
								// convert this into a fully qualified path name for the error message.
								NameOfDelegateFunction = FString(OwnerClass->GetName()) + TEXT(".") + NameOfDelegateFunction;
							}
							else
							{
								// convert this into a fully qualified path name for the error message.
								NameOfDelegateFunction = FString(SourceDelegateFunction->GetOwnerClass()->GetName()) + TEXT(".") + NameOfDelegateFunction;
							}
						}
						else
						{
							...
							SourceDelegateFunction = Cast<UFunction>(FindField(DelegateOwnerClass, *DelegateName, FALSE, UFunction::StaticClass(), NULL, NULL));
						}
...
)
{
	...
	{
		UStruct* SeekScope = TopNodeScope;
		if (SeekScope->GetClass() == UFunction::StaticClass())
			SeekScope = TopNodeScope->GetOwnerClass();

		Field = FindField(TopNodeScope, *VarType.Identifier);
		if (!Field)
		{
			if (TopNodeScope != TopNode)
				Field = FindField(Class, *VarType.Identifier);
			if (!Field)
				Field = FindObject<UField>(ANY_PACKAGE, *VarType.Identifier);
		}
		if (Cast<UEnum>(Field) != NULL)
		{
			// In-scope enumeration or struct.
			VarProperty = FPropertyBase((UEnum*)Field);
		}
		else if (Cast<UScriptStruct>(Field) != NULL)
		{
	...
	if ( !bReparsingClass )
	{
		...
		{
			GWarn->Logf(TEXT("You shouldn't use 'config' with object variable"));
		}
...
static UProperty* GroupProperty( UStruct* Scope, UProperty* Previous, QWORD PropertyFlags, FName ClassName )
{
	...
	if( bMovable )
	{
		for ( TFieldIterator<UProperty> It(Scope, FALSE); It; ++It )
		{
			...
			if( Property->HasAnyPropertyFlags(CPF_NoExport) && FString(Property->GetName()).Left(8) != TEXT("Vftable_") )
			{
				break;
			}
...
{
	static TMap<FString,INT> PreviousNames;
	FString PropertyStr(PropertyName);
	FString UpperPropertyStr = PropertyStr.Caps();
	...
	if (CurrentText != NAME_None &&
		appStrcmp(PropertyName,*CurrentText) != 0)
	{
...
)
{
	...
	if( HardcodedName )
	{
		...
		VarToken.Identifier = HardcodedName;
	}
	...
	if( !Skip )
	{
		...
		if ( Existing
		&&	(Cast<UFunction>(Scope) != NULL || Cast<UClass>(Scope) != NULL)		// declaring class member of function local/parm
					...
					else
					{
						...
						if ( Enum != NULL )
						{
							...
							else
							{
								...
								// subtract 1 because the last entry in the enum's Names array
								// is the _MAX entry
								VarProperty.ArrayDim = ArrayIndex = Enum->Names.Num() - 1;
							}
...
// 	if ( !Skip && !NoArrays && !IsFunction && Scope->GetClass() != UFunction::StaticClass() )
	if ( !bIsOperatorParm && !bIsFunctionLocal )
	{
	...
	FName PropertyName(*VarToken.Identifier, FindFlag);
	...
	else if( !Skip )
	{
		...
		else if( VarProperty.Type==CPT_Byte )
		{
			if ( Class->HasAnyClassFlags(CLASS_Native) && !HasMemberProperties(Scope) )
			{
				...
				if ( SuperScope != NULL )
				{
					...
					for( TFieldIterator<UProperty> It(SuperScope); It; ++It )
					{
			...
			if ( Array == NULL )
			{
				...
				Prev = GroupProperty( Scope, Prev, VarProperty.PropertyFlags, NAME_ByteProperty);
			}
		...
		else if( VarProperty.Type==CPT_Bool )
		{
			if ( Array == NULL )
			{
				...
				Prev = GroupProperty( Scope, Prev, VarProperty.PropertyFlags, NAME_BoolProperty);
			}
...
static void WriteFuncAssign(FScriptCompiler& Compiler, UObject* A, UObject* B, UObject* P)
{
	CodeSkipSizeType SkipSize = sizeof(ScriptPointerType) + 1 + 1 + sizeof(ScriptPointerType);
	UProperty* OuterProp = Compiler.GetUObjectOuterProperty();
	BYTE VarType = CPT_ObjectReference;

	Compiler.Writer << EX_Let;
	Compiler.Writer << EX_Context;
	Compiler.Writer << EX_ObjectConst;
	Compiler.Writer << A;
	Compiler.Writer << SkipSize << OuterProp << VarType;
	Compiler.Writer << EX_InstanceVariable;
	Compiler.Writer << P;
...
{
	INT iLine = InputLine;
	guard(CompileAffector);
	NBegin(CompileAffector);
	// Try to compile an affector expression or assignment.
	FScriptLocation LowRetry;
	GotAffector = 0;
	AffectorReturnProperty = NULL;
	...
	if( MatchSymbol(TEXT("=")) )
	{
		...
		if( !(ResultType.PropertyFlags & CPF_OutParm) )
		{
			...
			{
				GWarn->Logf(TEXT("'=': Left value is const"));
			}
			else if (ResultType.Type == CPT_ObjectReference && ResultType.TokenType == TOKEN_Const && ResultType.PropertyClass == UFunction::StaticClass())
			{
				// Compile right value.
				RequiredType = FPropertyBase(CPT_None, RightRefType);
				RequiredType.bIsLValue = TRUE;
				FToken ResultTypeB;
				
				if (CompileExpr(RequiredType, TEXT("'='"), &ResultTypeB) < 0 || ResultTypeB.Type != CPT_ObjectReference || ResultTypeB.TokenType != TOKEN_Const || ResultTypeB.PropertyClass != UFunction::StaticClass())
				{
					ScriptErrorf(SCEL_Unknown, TEXT("'=': Right value was not a function constant!"));
				}

				UObject* LFunc,*RFunc;
				ResultType.GetConstObject(UFunction::StaticClass(), LFunc);
				ResultTypeB.GetConstObject(UFunction::StaticClass(), RFunc);

				WriteFuncAssign(*this, LFunc, RFunc, GetFunctionHookHack());
#if WITH_FUNC_PARMS
				WriteFuncAssign(*this, LFunc, RFunc, ChildrenHack);
				WriteFuncAssign(*this, LFunc, RFunc, PropertyLinkHack);
				WriteFuncAssign(*this, LFunc, RFunc, ConstructorLinkHack);
#endif
				return;
			}
			else
		...
		&&	(ResultType.TokenProperty != NULL && (ResultType.TokenProperty->PropertyFlags&CPF_Const) == 0) )
		{
			...
			{
				GWarn->Logf(TEXT("%s: ProtectedWrite property is const here"), *ResultType.Identifier);
			}
			else if (ResultType.TokenProperty->PropertyFlags & CPF_PrivateWrite)
			{
				GWarn->Logf(TEXT("%s: PrivateWrite property is const here"), *ResultType.Identifier);
			}
			else
			{
				...
				for (INT i = TokenChain.Num() - 2; i >= 0; i--) // -2 as -1 should be the same as ResultType
				{
					if (TokenChain(i).TokenName == NAME_Default)
					{
						...
						GWarn->Logf(TEXT("'%s': Can't change default values of non-config properties"), *ResultType.Identifier);
					}
				}
	...
	else if( GotAffector )
	{
		...
		if (AffectorReturnProperty && ((AffectorReturnProperty->PropertyFlags & CPF_NeedCtorLink) || AffectorReturnProperty->GetSize() > 64))
		{
			FScriptLocation HighRetry;
	...
	for( ; ; )
	{
		...
		if( Token.Matches(NAME_Native) || Token.Matches(NAME_NativeOnly) )
		{
			...
			if( MatchSymbol(TEXT("(")) )
			{
				...
				{
					if (Token.Identifier == TEXT("inherit"))
					{
						UClass* SuperClass = Class->GetSuperClass();
						if ( SuperClass == NULL )
						{
		...
		else if( Token.Matches(NAME_Stripped) )
		{
			Class->TempCompileFlags |= TFFLAG_Stripped;
		}
		...
		else if( Token.Matches(NAME_Config) )
		{
			...
			if ( MatchSymbol(TEXT("(")) )
			{
				...
				else
				{
					...
					if (ConfigNameToken.Identifier == TEXT("inherit"))
					{
						UClass* SuperClass = Class->GetSuperClass();
						if ( SuperClass == NULL )
						{
...
{
	guard(::CompileInterfaceDeclaration);
	...
	for (;;) 
	{
		...
		if( Token.Matches(NAME_Native) || Token.Matches(NAME_NativeOnly) )
		{
			...
			if( MatchSymbol(TEXT("(")) )
			{
				...
				{
					if (HeaderToken.Identifier == TEXT("inherit"))
					{
						UClass* SuperClass = Class->GetSuperClass();
						if ( SuperClass == NULL )
						{
...
{
	guard(::CompileFunctionDeclaration);
	...
	for( ;; )
	{
		...
		{
			FuncInfo.TempCompileFlags |= TFFLAG_Stripped;
		}
		...
		else if ( Token.Matches(NAME_K2Call) )
		{
			FuncInfo.FunctionFlags |= FUNC_K2Call;
		}
		else if ( Token.Matches(NAME_K2Override) )
		{
			FuncInfo.FunctionFlags |= FUNC_K2Override;
		}
		else if ( Token.Matches(NAME_K2Pure) )
		{
			// This function can be called, and is also pure.
			FuncInfo.FunctionFlags |= FUNC_K2Call;
			FuncInfo.FunctionFlags |= FUNC_K2Pure;
		}
		else if (Token.Matches(NAME_Context))
		{
			RequireSymbol(TEXT("("), TEXT("'context'"));
			...
			if (ContextScope && MatchSymbol(TEXT(".")))
			{
				...
				UState* StateScope = ::FindField<UState>(ContextScope, *ContextToken.Identifier);
				if (!StateScope)
				{
					ContextScope = ::FindField<UFunction>(ContextScope, *ContextToken.Identifier);
					if (!ContextScope)
					{
						ScriptErrorf(SCEL_Restricted, TEXT("'Context(class.state.function)': Couldn't find function '%s'"), *ContextToken.Identifier);
					}
				}
				else if (MatchSymbol(TEXT(".")))
				{
					if (!GetIdentifier(ContextToken))
					{
						ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Missing function name"));
					}
					ContextScope = ::FindField<UFunction>(StateScope, *ContextToken.Identifier);
					if (!ContextScope)
					{
						ScriptErrorf(SCEL_Restricted, TEXT("'Context(class.state.function)': Couldn't find function '%s'"), *ContextToken.Identifier);
					}
			...
			FuncInfo.FunctionScope = ContextScope;
			RequireSymbol(TEXT(")"), TEXT("'Context(class.state.function)'"));
		}
#if WITH_LIBFFI
	...
	TCHAR* Signature = appStaticString1024();
		...
		if( !MatchSymbol(TEXT(")")) )
		{
			...
			do
			{
				...
				else
				{
					...
					appStrcpy( Temp, *GetPropertyName(Property.Type) );
	...
	{
		appStrncpy(Signature, *FuncInfo.Function.Identifier, 1024);
	}

	// Allocate local property frame, push nesting level and verify
	// uniqueness at this scope level.
	FName SignatureName(Signature, FNAME_Add);
	...
	if ( !bReparsingClass )
	{
		...
		TopFunction->TempCompileFlags	= FuncInfo.TempCompileFlags;
		TopFunction->OperPrecedence		= FuncInfo.Precedence;
		TopFunction->iNative			= FuncInfo.iNative;
		TopFunction->FriendlyName		= FName(*FuncInfo.Function.Identifier);
		TopFunction->CustomScope		= FuncInfo.FunctionScope;

		if ((FuncInfo.TempCompileFlags & TFFLAG_Stripped) && GEditor->bShouldObfuscate)
		{
			TopFunction->AltName = TopFunction->FriendlyName;
			TopFunction->Rename(UEditor::GetObscNameUnique(TopFunction->GetOuter()));
			TopFunction->FriendlyName = TopFunction->GetFName();
		}
	}
	...
	if( (TopFunction->FunctionFlags&FUNC_Delegate) != 0 && !bReparsingClass )
	{
		...
		FName MangledName = FName(*FString::Printf(TEXT("__%s__Delegate"),*TopFunction->FriendlyName), FNAME_Add);
...
void FScriptCompiler::CompileStateDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	UState* ParentState = Cast<UState>(FindField(TopNode, *NameToken.Identifier, TRUE, UState::StaticClass(), TEXT("'state'")));
	if( ParentState && ParentState->GetOwnerClass()==Class )
	{
		if ( !bReparsingClass )
		{
			ScriptErrorf(SCEL_Restricted,  TEXT("Duplicate state '%s'"), *NameToken.Identifier );
		}
	}
	...
	if( MatchIdentifier(NAME_Extends) )
	{
		...
		{
			ScriptErrorf(SCEL_Restricted,  TEXT("'Extends' not allowed here: state '%s' overrides version in parent class"), *NameToken.Identifier );
		}
		if( !GetIdentifier(ParentToken) )
		{
			ScriptErrorf(SCEL_Restricted,  TEXT("Missing parent state name") );
		}
		ParentState = Cast<UState>(FindField(TopNode, *ParentToken.Identifier, TRUE, UState::StaticClass(), TEXT("'state'")));
		if( !ParentState )
		{
	...
	FName StateName(*NameToken.Identifier);
...
void FScriptCompiler::CompileIgnoreDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	for( ; ; )
	{
		...
		if (IgnoredNames.FindItemIndex(IgnoreFunction.TokenName)>=0)
		{
				...
				for (TFieldIterator<UFunction> Function(Nest[i].Node); Function != NULL && !bFoundIgnoredFunc; ++Function)
				{
					if (Function->GetFName() == IgnoreFunction.TokenName)
					{
						...
						for( TFieldIterator<UProperty> It(*Function); It && (It->PropertyFlags & CPF_Parm); ++It )
						{
							...
							else if( Cast<UObjectProperty>(*It) )
							{
...
UBOOL FScriptCompiler::ParsePropertyMetaData(FPropertyBase& VarProperty, const TCHAR* PropertyTypeName)
{
	...
	if ( MatchSymbol(TEXT("<")) )
	{
		...
		PropertyMetaData.String.ParseIntoArray(&Pairs, TEXT("|"), TRUE);
...
void FScriptCompiler::CompileVariableDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	// Process all variables of this type.
	TArray<UProperty*> NewProperties;
	if (TopNest->NestType == NEST_Function && CastChecked<UFunction>(TopNode)->CustomScope && Cast<UFunction>(TopNode)->CustomScope->GetClass() == UFunction::StaticClass())
	{
		//GWarn->Logf(TEXT("Skipping local variables in %ls"), TopNode->GetFullName());
		...
		do
		{
			...
			//GWarn->Logf(TEXT("Skipping local variable %ls"), VarToken.Identifier);
			if (MatchSymbol(TEXT("[")))
			{
				if (!PeekSymbol(TEXT("]")))
				{
				...
				RequireSymbol(TEXT("]"), TEXT("'Array size'"));
			}
...
void FScriptCompiler::CompileCommand( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	else if( Token.Matches(NAME_Return) )
	{
		...
		UFunction* Function = (TopNodeScope && TopNodeScope->GetClass() == UFunction::StaticClass()) ? ((UFunction*)TopNodeScope) : CastChecked<UFunction>(Nest[i].Node);
...
//			FPropertyBase Property(Return,CPRT_AssignmentReference);
	...
	else if( MatchSymbol(TEXT(":")) )
	{
		...
		for( FLabelRecord *LabelRec = LabelNest->LabelList; LabelRec; LabelRec=LabelRec->Next )
		{
			...
			{
				ScriptErrorf(SCEL_Restricted,  TEXT("Duplicate label '%s'"), *Token.TokenName );
			}
		}
...
static void SkipEndOfDecl(FScriptCompiler* C)
{
	TCHAR T;
	while (1)
	{
...
static void SkipUntilChar(FScriptCompiler* C, TCHAR Test)
{
	TCHAR T;
	while (1)
	{
...
UBOOL FScriptCompiler::PreCompileStatements()
{
	guard(FScriptCompiler::PreCompileStatements);
	// Get a token and compile it.
	FToken Token;
	if (!GetToken(Token, NULL, 1))
	{
	...
	{
		if (Token.Matches(NAME_Class) || Token.Matches(NAME_Interface) || Token.Matches(NAME_Const))
		{
			SkipEndOfDecl(this); // Skip until end of section.
		}
		else if (Token.Matches(NAME_Var))
		{
			// Get category, if any.
			...
			while(1)
			{
				FToken Specifier;
				GetToken(Specifier);
				if (Specifier.Matches(NAME_Enum))
				{
					goto ParseEnumToken;
				}
				else if (Specifier.Matches(NAME_Struct))
				{
					goto ParseStructToken;
				}
				else if (Specifier.Matches(TEXT(",")))
					break;
				else if (Specifier.Matches(TEXT(";")))
				{
					UngetToken(Specifier);
		...
		ParseEnumToken:
			...
			// Verify that the enumeration definition is unique within this scope.
			UField* Existing = FindField(Class, *EnumToken.Identifier);
			if (Existing && Existing->GetOuter() == Class && !bReparsingClass)
				ScriptErrorf(SCEL_Unknown/*SCEL_Parse*/, TEXT("enum: '%s' already defined here"), *EnumToken.TokenName);

			if (!bReparsingClass)
			{
				// Create enum definition.
				UEnum* Enum = new(Class, *EnumToken.Identifier, RF_Public) UEnum();
			}
		...
		ParseStructToken:
			...
			while(1)
			{
				...
				if (StructToken.TokenName == NAME_Native || StructToken.TokenName == NAME_Export || StructToken.TokenName == NAME_Transient
					|| StructToken.TokenName == NAME_Atomic || StructToken.TokenName == NAME_Immutable || StructToken.TokenName == NAME_ImmutableWhenCooked || StructToken.TokenName == NAME_StrictConfig)
				{
				}
				else break;
			}
			...
			// Verify uniqueness.
			UField* Existing = FindField(Class, *StructToken.Identifier);
			if (Existing && Existing->GetOuter() == Class && !bReparsingClass)
			{
				ScriptErrorf(SCEL_Unknown, TEXT("struct: '%s' already defined here"), *StructToken.TokenName);
			}
			...
			{
				// Create.
				UScriptStruct* Struct = new(Class, *StructToken.Identifier, RF_Public)UScriptStruct(nullptr);
			}
			SkipUntilChar(this, '{');
			SkipUntilChar(this, '}');
			SkipEndOfDecl(this); // Skip until end of section.
		}
		else if (IsValidFunctionSpecifier(Token)) // Stop once we reach functions.
			return 0;
		else if (Token.Matches(NAME_State) || Token.Matches(NAME_Auto) || Token.Matches(NAME_Simulated) || Token.Matches(NAME_Replication)) // End if we reach states or replication block.
			return 0;
		else if (Token.Matches(TEXT("#")))
		{
			// Compiler directive.
		...
		else return 0; // Unknown command or expression, let parser handle with this.
	}
	return 1;
	unguard;
}
...
void FScriptCompiler::PostParse( UStruct* Node )
{
	...
	if  ( Struct != NULL )
	{
		const INT Size = Struct->GetPropertiesSize() - Struct->GetDefaultsCount();
		if( Size > 0 )
		{
...
struct FOldCodeReader : public FArchive
{
private:
	BYTE* oCode;

public:
	FOldCodeReader(BYTE* c)
		: oCode(c)
	{}
	void Serialize(void* V, INT Length)
	{
		appMemcpy(V, oCode, Length);
		oCode += Length;
	}
	inline BYTE ReadCode()
	{
		return *oCode++;
	}
	inline BYTE GetLastCode()
	{
		return oCode[-1];
	}
};
...
void FScriptCompiler::CompileSecondPass( UStruct* Node )
{
	...
	UFunction* TopFunction = Cast<UFunction>(TopNodeFunc);
	if( TopFunction )
	{
		...
		try
		{
			if (TopNodeFunc != TopNode)
			{
				// Hack! read optional values from original code.
				FOldCodeReader opCode(&TopFunction->Script(0));
				for (TFieldIterator<UProperty> It(TopFunction); It; ++It)
				{
					UProperty* prop = *It;
					if ((prop->PropertyFlags & CPF_OptionalParm) != 0)
					{
						if (opCode.ReadCode() == EX_DefaultParmValue)
						{
							Writer << EX_DefaultParmValue;

							CodeSkipSizeType SkipOffset;
							opCode << SkipOffset;
							Writer << SkipOffset;

							for (WORD iSkip = 0; iSkip < SkipOffset; ++iSkip)
							{
								BYTE CodeToken = opCode.ReadCode();
								Writer << CodeToken;
							}

							checkf(opCode.GetLastCode() == EX_EndParmValue, TEXT("LastCode: %ls (%i)"), GetExprName(opCode.GetLastCode()), INT(opCode.GetLastCode()));
						}
	...
	if( Node==Class && Node->Line!=INDEX_NONE )
	{
		...
		try
		{
			while( 1 )
			{
				...
				if (!Token.Matches(NAME_If))
				{
					ScriptErrorf(SCEL_NestLevel, TEXT("Missing 'if' in replication statement"));
				}
				...
				do
				{
					...
					for( TFieldIterator<UProperty> It(Class); It && It.GetStruct()==Class; ++It )
					{
						if( It->GetFName()==VarName )
						{
							...
							{
								ScriptErrorf(SCEL_Restricted,  TEXT("Variable '%s' already has a replication definition"), *VarName );
							}

							// Make sure it isn't a dynamic array property
							if ( It->IsA(UArrayProperty::StaticClass()) )
							{
								ScriptErrorf(SCEL_Restricted, TEXT("Dynamic arrays cannot be replicated '%s'"), *VarName);
							}
					...
					if( !Found )
					{
						ScriptErrorf(SCEL_Unknown,  TEXT("Bad variable or '%s' in replication definition"), *VarName );
					}
				} while( MatchSymbol( TEXT(",") ) );
	...
	unguardf((TEXT("(%ls)"),Node->GetFullName()));
}
...
)
{
	...
	if (InPass != PASS_PreProcessStructs)
	{
	...
	try
	{
		...
		{
			GLog->Logf(TEXT("Parsing %ls..."), Class->GetName());
			guard(ParseCodes);
			// Parse entire program.
		...
		{
			GLog->Logf(TEXT("Compiling %ls..."), Class->GetName());
			guard(CompileCodes);
		...
		if (InPass == PASS_Parse && !bReparsingClass)
		{
			guard(PostParse);
			// Finish parse.
			PostParse( Class );
			...
			if( (Class->ClassFlags & CLASS_Config) && (Class->ClassConfigName == NAME_None) )
			{
				...
				{
					GWarn->Logf(TEXT("%ls: Classes with config / globalconfig member variables need to specify config file."), Class->GetFullName());
...
//
void FScriptCompiler::AddResultText(const TCHAR* Fmt, ...)
{
	INT		BufSize = 1024;
	TCHAR* Buffer = NULL;
	INT		Result = -1;
	...
	while (Result == -1)
	{
		...
		{
			GLog->Logf(TEXT("Illegal FScriptCompiler::AddResultText (error: %ls)"), appFromAnsi(strerror(errno)));
			Result = 0;
			break;
		}
		BufSize <<= 1;
 ...
 */
static UBOOL ParseScripts( UClass* Class, TArray<UClass*>& AllClasses, FScriptCompiler& Compiler )
{
	if (!Class->ScriptText)
		return TRUE;
	...
	{
		UClass* OrigDependsOnClass = FindClass(*Class->DependentOn(NameIndex));
		UClass* DependsOnClass = OrigDependsOnClass;
		if (!DependsOnClass)
		{
			Compiler.ScriptErrorf(SCEL_Class, TEXT("Failed to find DependsOn/Implements class '%s' while parsing '%s'"), *Class->DependentOn(NameIndex), Class->GetName());
			return FALSE;
		}
		else
		{
			...
			for (UClass* ParentClass = DependsOnClass->GetSuperClass(); ParentClass && !ParentClass->HasAnyClassFlags(CLASS_Parsed); ParentClass = ParentClass->GetSuperClass())
			{
			...
			while (ClassesToParse.Num() > 0)
			{
				...
				if (ParseScripts(NextClass, AllClasses, Compiler))
				{
					break;
				}
				else
	...
	if (!(Class->ClassFlags & CLASS_Parsed))
	{
		...
		// Compile parent class if not yet done.
		if (CurrentSuperClass && !(CurrentSuperClass->ClassFlags & CLASS_Parsed) && AllClasses.FindItemIndex(CurrentSuperClass)>=0 && !ParseScripts(CurrentSuperClass, AllClasses, Compiler))
			return FALSE;

		if (!Compiler.CompileScript(AllClasses, Class, &GMem, 1, PASS_Parse))
		{
...
static INT CountClassesWithUnparsedDefaults( const TArray<UClass*>& AllClasses )
{
	INT Result = 0;
	for (INT i = 0; i < AllClasses.Num(); ++i)
	{
		if (AllClasses(i)->ClassFlags & CLASS_NeedsDefProps)
			++Result;
	}
	return Result;
}

static void GetClassesWithUnparsedDefaults( const TArray<UClass*>& AllClasses, FString& Result )
{
	for (INT i = 0; i < AllClasses.Num(); ++i)
	{
		if (AllClasses(i)->ClassFlags & CLASS_NeedsDefProps)
			Result += FString::Printf(Result.Len() ? TEXT(", %s") : TEXT("%s"), AllClasses(i)->GetName());
	}
}

static FString GetClassesWithUnparsedDefaults( const TArray<UClass*>& AllClasses )
{
	FString Result;
...
UBOOL UEditor::ImportDefaultProps(UClass* Class)
{
	guard(UEditor::ImportDefaultProps);
	UClass* SuperClass = Class->GetSuperClass();
	if (SuperClass && (SuperClass->ClassFlags & CLASS_NeedsDefProps) && !ImportDefaultProps(SuperClass))
		return FALSE;
	...
	if (Class->ClassFlags & CLASS_NeedsDefProps)
	{
		...
		DefaultObject->InitClassDefaultObject(Class);
		if (SuperClass)
		{
		...
		INT ErrorCount = GWarn->GetErrors();

		UProperty::_ImportObject = Class;
		const TCHAR* ResultBuffer = ImportDefaultProperties(
			...
			0,
			LineNumber);
		UProperty::_ImportObject = nullptr;
 ...
 */
UBOOL FScriptCompiler::CompileDefaultProperties( const TArray<UClass*>& AllClasses )
{
	UBOOL bSuccess = TRUE;
	...
	for (INT i = 0; i < AllClasses.Num(); ++i)
	{
		GWarn->Logf(NAME_Title, TEXT("Importing class defaults (%i/%i)"), (i + 1), AllClasses.Num());
		if (!GEditor->ImportDefaultProps(AllClasses(i)))
			bSuccess = FALSE;
	}
 ...
 * @return	TRUE if the class defaults were successfully parsed
 */
UBOOL FScriptCompiler::CompileClassDefaults( const TArray<UClass*>& AllClasses )
{
	INT InitialErrorCount = GWarn->GetErrors();
	INT LastCount = CountClassesWithUnparsedDefaults( AllClasses );
	do
	{
		if( !CompileDefaultProperties( AllClasses ) )
		{
			return 0;
		}
	...
	return Warn->GetErrors() == InitialErrorCount;
}

static UBOOL CompileStructDefaults( TArray<UScriptStruct*>& ScriptStructs, FScriptCompiler& Compiler );
static void CompileStructDefaults( UScriptStruct* StructToCompile, TArray<UScriptStruct*>& ScriptStructs, FScriptCompiler& Compiler )
{
	if ( StructToCompile )
	{
		//if(StructToCompile->GetClass()==UClass::StaticClass())
		//	GWarn->Logf(TEXT("Imporing defaultproperties for %ls..."), StructToCompile->GetName());
		...
		if ( ParentStruct )
		{
			...
			if (ScriptStructs.FindItemIndex(ParentStruct) >= 0)
				CompileStructDefaults(ParentStruct, ScriptStructs, Compiler);
		...
		for( TFieldIterator<UStructProperty> It(StructToCompile); It; ++It )
		{
			...
			if ( (NestedStructProperty->PropertyFlags&CPF_Native) == 0 )
			{
				...
				if (ScriptStructs.FindItemIndex(NestedStruct) >= 0)
					CompileStructDefaults(NestedStruct, ScriptStructs, Compiler);
			}
		}
...
static UBOOL CompileStructDefaults( TArray<UScriptStruct*>& ScriptStructs, FScriptCompiler& Compiler )
{
	guard(CompileStructDefaults);
	INT InitialErrorCount = GWarn->GetErrors();

	while ( ScriptStructs.Num() > 0 )
	{
		UScriptStruct* Struct = ScriptStructs.Pop();
		CompileStructDefaults(Struct, ScriptStructs, Compiler);
	}

	// return success if the error count didn't increase!
	return GWarn->GetErrors() == InitialErrorCount;
	unguard;
}

static void ClearDefaultPropertyText( const TArray<UClass*>& AllClasses )
{
	guard(ClearDefaultPropertyText);
	for(INT i=0; i< AllClasses.Num(); ++i)
		AllClasses(i)->DefaultPropText.Empty();
	unguard;
}
...
// Not recursive.
//
UBOOL UEditor::MakeScripts(FFeedbackContext* Warn, UPackage* MyPackage, TArray<UClass*>& AllClasses)
{
	guard(UEditor::MakeScripts);
	INT i;
	for(i=0; i< AllClasses.Num(); ++i)
		AllClasses(i)->ClassFlags &= ~(CLASS_Parsed | CLASS_Compiled);

	guard(InitMetaManager);
	if (!GScriptHelper)
		GScriptHelper = new FCompilerMetadataManager;
	unguard;

	FMemMark Mark(GMem);
	FScriptCompiler	Compiler(Warn, 0);
	Compiler.InDefaultPropContext = 0;
	...
	try
	{
		for (i = 0; i < AllClasses.Num(); ++i)
		{
			GWarn->Logf(NAME_Title, TEXT("Pre-Build step (%i/%i)"), (i+1), AllClasses.Num());
			if (!Compiler.CompileScript(AllClasses, AllClasses(i), &GMem, 1, PASS_PreProcessStructs))
				Success = FALSE;
		}
		for (i = 0; i < AllClasses.Num(); ++i)
		{
			GWarn->Logf(NAME_Title, TEXT("Parsing (%i/%i %ls)"), (i + 1), AllClasses.Num(), AllClasses(i)->GetName());
			if (!ParseScripts(AllClasses(i), AllClasses, Compiler))
				Success = FALSE;
		}
	}
	catch ( TCHAR* ErrorMsg )
	...
	if (Success)
	{
		for (i = 0; i < AllClasses.Num(); ++i)
		{
			GWarn->Logf(NAME_Title, TEXT("Compiling (%i/%i %ls)"), (i + 1), AllClasses.Num(), AllClasses(i)->GetName());
			if (!CompileScripts(AllClasses, Compiler, AllClasses(i)))
				Success = FALSE;
		}
		if ( Success )
		{
			...
			for( TObjectIterator<UScriptStruct> ObjIt; ObjIt; ++ObjIt )
				if (ObjIt->IsIn(MyPackage))
					ScriptStructs.AddUniqueItem(*ObjIt);

			try
			{
				GWarn->Log(NAME_Title, TEXT("Importing struct defaults"));
				Success = CompileStructDefaults( ScriptStructs, Compiler );
				Success = Success && Compiler.CompileClassDefaults( AllClasses );
			}
			catch ( TCHAR* ErrorMsg )
			{
				Warn->Log( NAME_Error, ErrorMsg );
	...
	else GWarn->Log(TEXT("Compilation aborted due to errors!"));
