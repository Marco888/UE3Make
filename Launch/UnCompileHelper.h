...
#define MAX_STATICSTRLEN 8192

extern class FCompilerMetadataManager* GScriptHelper;

#define CASE_NAME(nn) case nn: return TEXT(#nn)
...
#define COMP_DEBUG_LOG 0

#if COMP_DEBUG_LOG
// Debug logger
struct FDebugLogger : public FOutputDevice
{
	static FDebugLogger* DebugLog;
	INT Depth;
	INT MemUse;
	...
	void BeginSubRoutine(const TCHAR* Name);
	void EndSubRoutine();
};
struct FSubRoutineScope
{
	FSubRoutineScope(const TCHAR* N)
	{
		FDebugLogger::DebugLog->BeginSubRoutine(N);
	}
	~FSubRoutineScope()
	{
		FDebugLogger::DebugLog->EndSubRoutine();
	}
};
struct FMemCalcRoutine
{
	INT Num;
	FMemCalcRoutine(INT sz)
		: Num(sz + sizeof(INT))
	{
		FDebugLogger::DebugLog->MemUse += Num;
	}
	~FMemCalcRoutine()
	{
		FDebugLogger::DebugLog->MemUse -= Num;
...
#define NMemCount(x) FDebugLogger::DebugLog->MemUse+=x
#define NBegin(x) FSubRoutineScope RouteScope(TEXT(#x))
#define NLog(x) FDebugLogger::DebugLog->Log(x)
#define NLogf(x) FDebugLogger::DebugLog->Logf x
#else
#define NMemCount(x) 
#define NBegin(x)
#define NLog(x) 
...
public:
	...
	, BitMask(0), MetaClass(NULL), ArrayIndexEnum(NULL), DelegateName(NAME_None), ReferenceType(InRefType), bIsLValue(FALSE)
	{
	}
	...
	, Enum(InEnum), MetaClass(NULL), ArrayIndexEnum(NULL), DelegateName(NAME_None), ReferenceType(InRefType), bIsLValue(FALSE)
	{
	}
	...
	, PropertyClass(InClass), MetaClass(InMetaClass), ArrayIndexEnum(NULL), DelegateName(NAME_None), ReferenceType(InRefType), bIsLValue(FALSE)
	{
	...
	, Struct(InStruct), MetaClass(NULL), ArrayIndexEnum(NULL), DelegateName(NAME_None), ReferenceType(InRefType), bIsLValue(FALSE)
	{
	}
	FPropertyBase( UProperty* Property, EPropertyReferenceType InRefType = CPRT_None )
	: PropertyExportFlags(PROPEXPORT_Public), DelegateName(NAME_None), bIsLValue(FALSE)
	{
		checkSlow(Property);
...
inline const TCHAR* GetTokenTypeName(ETokenType TT)
{
	switch (TT)
	{
		CASE_NAME(TOKEN_None);
		CASE_NAME(TOKEN_Identifier);
		CASE_NAME(TOKEN_Symbol);
		CASE_NAME(TOKEN_Const);
	default: return TEXT("TOKEN_UNKNOWN");
	}
}
...
public:
	...
	FString String;								 // If CPT_String or IsPointer().
	...
	inline const TCHAR* Desc() const
	{
		TCHAR* Result = appStaticString1024();
		appSprintf(Result, TEXT("(TokenType=%ls Name=%ls Identifier=%ls)"), GetTokenTypeName(TokenType), *TokenName, *Identifier);
		return Result;
	}
	...
	FString GetValue()
	{
		if ( TokenType == TOKEN_Const )
		{
			...
			else
			{
				...
				case CPT_Bool:				return FString::Printf(TEXT("%s"), Bool ? *FName(NAME_True) : *FName(NAME_False));
				case CPT_Float:				return FString::Printf(TEXT("%f"), Float);
				case CPT_Name:				return FString::Printf(TEXT("%s"), *NameBytes);
	...
	inline void ZeroData()
	{
		...
		NameBytes = NAME_None;
		String.Empty();
		VectorBytes = FVector(0,0,0);
		RotationBytes = FRotator(0,0,0);
		Interface = 0;
	}
	...
	{
		return (TokenType==TOKEN_Identifier || TokenType==TOKEN_Symbol) && Identifier==Str;
	}
	...
	UBOOL GetConstByte( BYTE& B ) const
	{
		...
		else if( TokenType==TOKEN_Const && Type==CPT_Float && Float>=0 && Float<255 && Float== appFloor(Float))
		{
			B = (BYTE)appFloor(Float);
			return 1;
		}
		else return 0;
	}
	UBOOL GetConstInt( INT& I ) const
	{
		...
		else if( TokenType==TOKEN_Const && Type==CPT_Float && Float==appFloor(Float))
		{
			I = appFloor(Float);
			return 1;
		}
	...
	FString Describe()
	{
		return FString::Printf(
			...
			GetPropertyTypeText(Type), *TokenName, *GetValue(),
			Struct != NULL ? Struct->GetName() : TEXT("NULL"),
...
struct FFuncInfo
{
	...
	,	FunctionScope	(Other.FunctionScope)
	{
