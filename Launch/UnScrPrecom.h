...
public:
	...
	*/
	FString CurrentLocation() const
	{
		if (macroName_ != TEXT("")) 
		{
			return FString::Printf(TEXT("%s(%d)"), *macroName_, InputLine+1);
		}
	...
	*/
	...
	void ParseMacroParameterValues(const TArray<FString>* ParamNames, TArray<FString>* ParamValues) 
	{
		...
		SymbolMap->Set(TEXT("#"), FScriptMacroDefinition(ParamNames && ParamNames->Num() > 0 ? *FString::Printf(TEXT("%i"),ParamNames->Num()) : TEXT("0")));
	}
...
public:
	...
	*/
	...
	{
		ProviderStack.AddItem(new FCharacterProvider(File_BufferStart, File_BufferEnd));
	}
	...
	*/
	...
	{
		ProviderStack.AddItem(new FCharacterProvider(File_BufferStart, File_BufferEnd, Filename, LineNumber));
	}
	...
	void AddMacro(const TCHAR* Macro_BufferStart, const TCHAR* Macro_BufferEnd, const FName& MacroName, FScriptMacroDefinition* MacroDef, TArray<FString>* Values) 
	{
		...
		if ( NewProvider != NULL )
		{
			ProviderStack.AddItem(NewProvider);
...
public:
	...
	*/
	...
	{
		return EnableOutputFlag.FindItemIndex(0) == INDEX_NONE;
	}
	 ...
	 */
	void ProcessIncludeFile( const TCHAR* IncludeFilename, FString* FileContent=NULL );
